...
bool GUIRectangle::loadXMLSettings(XMLElement *element)
{
  ...
  if((attr = element->getAttributeByName("callbackString")) ||
     (attr = element->getAttributeByName("name")          ))
    setCallbackString(attr->getValuec());
  else
    return false;//LOG_PRINT_NONE("Need a <name> or <callbackString> attribute, check ")
                   //              + element->getName());

  if(attr = element->getAttributeByName("anchorPoint"))
    setAnchorPoint(attr->getValuec());

  if(attr = element->getAttributeByName("visible"))
    setVisible(attr->getValue() == "true");

  if(attr = element->getAttributeByName("active"))
    setActive(attr->getValue() == "true");

  if(child = element->getChildByName("Position"))
  {
    XMLElement::loadRX_GYf(*child, position);
    LOG_FNLN;
    LOG_PRINT("position.x=%f, position.y=%f\n", position.x, position.y);
  }
...
const void GUIRectangle::computeWindowBounds()
{
  if(parent && update)
  {
    ...
    #if 0
    LOG_FNLN;
    LOG_PRINT("windowBounds.x=%f\n", windowBounds.x);
    LOG_PRINT("windowBounds.y=%f\n", windowBounds.y);
    LOG_PRINT("windowBounds.z=%f\n", windowBounds.z);
    LOG_PRINT("windowBounds.w=%f\n", windowBounds.w);
    #endif
  }
}
