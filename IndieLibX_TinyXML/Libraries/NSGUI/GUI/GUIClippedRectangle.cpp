...
bool GUIClippedRectangle::loadXMLClippedRectangleInfo(XMLElement *element)
{
  ...
  if(attr = element->getAttributeByName("clipSize"))
    clipSize = attr->getValuei();

  if(child = element->getChildByName("BordersColor"))
    XMLElement::loadRX_GY_BZf(*child, bordersColor);

  if(attr = element->getAttributeByName("drawBackground"))
    enableBGColor(attr->getValue() == "true");

  if(attr = element->getAttributeByName("drawBounds"))
    setVisibleBounds(attr->getValue() == "true");
