...
public:
    ...
    GLuint createNoiseTexture3D(int w, int h, int d, float scale, float amp, NoiseMode noiseMode, glh::tex_object_3D &m_noisetex)
    {
      ...
      PROGRESSHIDE();
      return m_noisetex.texture;
    }
	// Create an OpenGL 3D texture containing filtered 3D (un-normalized vector) noise
	void createVectorNoiseTexture3D(int w, int h, int d, float scale, float amp, NoiseMode noiseMode, glh::tex_object_3D &m_noisetex)
	{
	  GLfloat *img = new GLfloat[w * h * d * 3];
...
public:
	...
	**/ //----------------------------------------------------------------------------------
    ...
    GLuint createPerlinTexture3D_Luminance(int w, int h, int d, glh::tex_object_3D &m_noisetex, float amp=1, PerlinMode noiseMode=RAW)
    {
      GLubyte *img = new GLubyte[w * h * d];
    ...
    GLuint createPerlinTexture3D_RGB(int w, int h, int d, glh::tex_object_3D &m_noisetex, float amp=1, PerlinMode noiseMode=RAW)
    {
      GLubyte *img = new GLubyte[w * h * d * 3];
...
/*			switch(noiseMode)
            ...
            *p++ = (GLubyte) (v3[0] * amp * 0xff);
            *p++ = (GLubyte) (v3[1] * amp * 0xff);
            *p++ = (GLubyte) (v3[2] * amp * 0xff);
          }
        }
      }
