...
HDRtexture::HDRtexture(char *filename)
	: m_valid(false)
{
	path.path.push_back("");
	path.path.push_back(".");
	path.path.push_back("../../../MEDIA/textures/");
	path.path.push_back("../../../../MEDIA/textures/");

  FILE *fp = path.fopen(filename, "rb");
  if (!fp) {
...
//  if (RGBE_ReadPixels_RLE(fp, img.data, img.width, img.height))
  ...
  m_valid = true;
  LOG_PRINT("Loaded HDR image '%s' %d x %d\n", filename, m_width, m_height);
}
...
HDRtexture::Create2DTextureRGBE(GLenum target)
{
  ...
  glTexParameteri(m_target, GL_TEXTURE_WRAP, GL_CLAMP_TO_EDGE);
...
HDRtexture::CreateCubemapHILO(bool rg)
{
  ...
  glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X_ARB, 0, GL_HILO16_NV, face_width, face_height, 0, GL_HILO_NV, GL_FLOAT, face);
  ...
  glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y_ARB, 0, GL_HILO16_NV, face_width, face_height, 0, GL_HILO_NV, GL_FLOAT, face);
  ...
  glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z_ARB, 0, GL_HILO16_NV, face_width, face_height, 0, GL_HILO_NV, GL_FLOAT, face);
