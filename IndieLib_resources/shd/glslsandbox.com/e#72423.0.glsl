...
#define iTime time
#endif
uniform vec3 iResolution_ vec3(resolution,1.)
uniform vec4 iMouse_ = vec4(0.);
#endif
...
void mainImage_(out vec4 frag_col, in vec2 frag_coord) {
  vec2 res = demos_redlandscape_vec3_xy(iResolution_);
  vec2 mouse = (demos_redlandscape_vec4_xy(iMouse_) / res);
