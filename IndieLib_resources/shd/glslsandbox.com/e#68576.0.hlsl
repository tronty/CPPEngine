...
float4 main(VsOut IN): COLOR 
{
    ...
    // Light vector
    float3 l = normalize(float3(0, 0.2, 1.0)); // 

    // Render Sky
    float sundot = clamp(dot(rd, l), 0.0, 1.0);
    float skyGrad = pow(sundot, 800.0) * 1.0 + pow(sundot, 10.0) * 0.18 + pow(sundot, 0.5) * 0.05;
    float skyGradY1 = smoothstep(-1.2, 0.5, p.y + pow(abs(p.x), 2.0) * 0.3 - 0.5);
    float skyGradY2 = smoothstep(-0.3, 0.2, p.y + abs(p.x) * 0.2);
    float3 skyBgColor = lerp(SKY_COLOR_HORIZON_1, SKY_COLOR, skyGradY1);
    skyBgColor = lerp(SKY_COLOR_HORIZON_2, skyBgColor, skyGradY2);

    float3 sun = SUN_COLOR_2 *  pow(sundot, 20.0) * SUN_COLOR_SCALE;
    float3 skyColor = lerp(skyBgColor, sun, skyGrad);
