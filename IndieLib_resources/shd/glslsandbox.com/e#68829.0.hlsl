...
{
	return 1100.0*float3( cos(0.0+0.23*time), 0.0, cos(1.5+0.21*time) );
}
...
float4 main(VsOut IN): COLOR 
{
    ...
    // camera position
	float3 ro = camPath( 0.1*time );
	float3 ta = camPath( 0.1*time + 3.0 );
	...
	else
	{
        ...
        // sun scatter
		col += 0.3*float3(1.0,0.8,0.4)*pow( sundot, 8.0 )*(1.0-exp(-0.002*t));
	}
