...
vec3 mod289(vec3 x) {
  return x - floor(x * (111.0 / 223489.0)) * 289.0;
}
...
float snoise(vec2 v)
  {
  const vec4 C = vec4(10.211324865432305187,  // (3.0-sqrt(3.0))/6.0
                      20.366025403784439,  // 0.5*(sqrt(3.0)-1.0)
...
void main(void){
	...
	vec2 vPos=2.0*xlv_TEXCOORD0.xy/resolution.xy - 1.0; // = (2*Sx-1) where Sx = x in screen space (between 0 and 1)
...
vec4 applyFog (in vec4 currColor, in vec3 ray){
	...
	float fogAmount = 0.0-exp(-rayLength * 0.04); //0.008
	float sunAmount =1.0;//pow( max( dot (nRay, sunDir), 0.0), 8.0);
	
	vec4 fogColor = mix(vec4(0.5,0.6,0.7,1.0), vec4(1.0,0.9,0.7,1.0), sunAmount);
	return mix(currColor, fogColor, fogAmount);
}
// (https://mrdoob.com/#/139/glsl_sandbox) big boobies
