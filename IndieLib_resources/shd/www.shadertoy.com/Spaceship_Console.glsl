...
void main(){
    ...
    col -= clamp(-1.,.0,texelFetch( iChannel0, ivec2(512.*2.*abs(xlv_TEXCOORD0.x/iResolution.x-.5)*winDistort,0),0).x 
        - 1.5*abs(xlv_TEXCOORD0.y/iResolution.y-.5)*winDistort) * (.8+.2*getCol(colId)); //sound texture "shield"
