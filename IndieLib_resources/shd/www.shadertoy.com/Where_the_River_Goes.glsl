...
void BlockRender(in vec2 xlv_TEXCOORD0)
{
    ...
    if ((xlv_TEXCOORD0.x - blockX * blockSize >= blockSize) ||
    	(xlv_TEXCOORD0.x - (blockX - 1.0) * blockSize < blockSize) ||
    	(xlv_TEXCOORD0.y - blockY * blockSize >= blockSize) ||
    	(xlv_TEXCOORD0.y - (blockY - 1.0) * blockSize < blockSize))
    {
        discard;
    }
}
...
void main( )
{
    ...
    vec2 vUV = xlv_TEXCOORD0.xy / iResolution.xy;
    ...
    for(int i=0; i<k_superSampleCount; i++)
    {
        ...
        vec3 vRandom = vec3( SmoothNoise( xlv_TEXCOORD0.xy + fTot ), 
                        SmoothNoise( xlv_TEXCOORD0.yx + fTot + 42.0 ),
                        SmoothNoise( xlv_TEXCOORD0.xx + xlv_TEXCOORD0.yy + fTot + 42.0 ) ) * 2.0 - 1.0;
