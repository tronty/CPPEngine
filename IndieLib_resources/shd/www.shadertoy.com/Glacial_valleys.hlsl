...
#define OCTAVES 8
float fbm(in float2 uv,int octaves)
{
    ...
    for (int i = 0; i < octaves; i++)
    {
        ...
        uv = uv.yx + float2(freq,amplitude-n1/freq);
    }
...
float4 main(VsOut IN): COLOR 
{
    ...
    float3 camStep = float3(lightDir.x, 0., lightDir.z) * (iTime+12.)/4.;
