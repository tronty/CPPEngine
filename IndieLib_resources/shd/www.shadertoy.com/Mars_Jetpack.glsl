...
// Uncomment this for a faster landscape that uses a texture2D for the fractal:-
...
vec3 NoiseD( in vec2 p )
{
    ...
    vec4 n;
	n.x = texture2DLod( iChannel0, (p+vec2(0.5,0.5))*STEP, 0.0 ).x;
	n.y = texture2DLod( iChannel0, (p+vec2(1.5,0.5))*STEP, 0.0 ).x;
	n.z = texture2DLod( iChannel0, (p+vec2(0.5,1.5))*STEP, 0.0 ).x;
	n.w = texture2DLod( iChannel0, (p+vec2(1.5,1.5))*STEP, 0.0 ).x;

    // Normally you can make a texture2D out of these 4 so
...
void main( )
{
    ...
    vec2 xy = xlv_TEXCOORD0.xy / iResolution.xy;
	vec2 uv = (-1.0 + 2.0 * xy) * vec2(iResolution.x/iResolution.y,1.0);
    coord = xlv_TEXCOORD0 / iChannelResolution[0].xy;
	vec3 camTar;
	
	float hTime = mod(gTime+1.95, 2.0);
	
	#ifdef STEREO
	float isRed = mod(xlv_TEXCOORD0.x + mod(xlv_TEXCOORD0.y,2.0),2.0);
	#endif
