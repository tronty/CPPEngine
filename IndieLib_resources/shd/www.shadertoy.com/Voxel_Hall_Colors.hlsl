...
/*
    ...
    I removed the tex2D and added a surface color with a time gradient.
...
float3 tex3D( sampler2D tex, in float3 p, float3 n ){
...
float4 main(VsOut IN): COLOR 
{
	...
	if(t<FAR){
        ...
        // Object texturing.
        float3 tint;
        //tint = float3(.94, .38, .57); // rose
        //tint = float3(1, .6, 1.); // lavender
        //tint = float3(.5); // greyscale
        //tint =  .7 + .5*cos(6.28318*(float3(1.,0.1,0.4)*iTime*.25 + float3(0.5,0.15,0.25))) ;
        tint = .7 + .5*cos(6.28318*(float3(2.0,1.,0.)*iTime*.25 + float3(.5,.2,.25)));
        float3 texCol = tint + step(abs(snNoBump.y), .5);

    	// Combining the above terms to produce the final color. It was based more on acheiving a
        // certain aesthetic than science.
        sceneCol = texCol*(diff + ambience) + float3(1., 1., 1.) *spec;
