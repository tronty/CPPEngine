...
// tweaking explosion for rasterization on STM32

// fork of shader "Volumetric Explosion" by the Duke

// used in rasterization of part of the explosions. see gif below (capture from 256x64 4bit grayscale oled display)

// (stm32 hobby project)
// more @ https://supersinfulsilicon.orgfree.com/stm32.html
// all open source

// original shader by the Duke :
...
float normalizer = 1.0 / 4.1231056256176605498214098559741f;	// pythagorean theorem on that perpendicular to maintain scale
...
float VolumetricExplosion(vec3 p)
{
   ...
   // final += fbm(p*50.);
    #endif
    final += SpiralNoiseC(vec3(p.xy,p.z)*0.4132+333.)*2.4f; //1.25;
...
float map(vec3 p) 
{
	//R(p.xy, iMouse.x*0.008*pi+iTime*0.1);

	float VolExplosion = VolumetricExplosion(p*2.0f)*0.5; // scale
...
void main( )
{
   ...
   // key += 0.7*texture(iChannel1, vec2(KEY_3,0.25)).x;
    ...
    #endif
	{
	    ...
	    if(td>0.9 || d<0.12*t || t>10. || t>max_dist) break;
       ...
       // vec3 lightColor=vec3(1.0,0.5,0.25);
        ...
        sum += (exp(-lDist*lDist*lDist*.08)/30.); // bloom
   ...
   // sum *= 1. / exp( ld * 0.2 ) * 0.9;
    ...
    //.xyz = sum.xyz*sum.xyz*(3.0-2.0*sum.xyz);
...
#else
    float cd = (td-ld) * (1.0f - sum);
     gl_FragColor = vec4(cd,cd,cd,1.0);
	#endif
	#endif
}
