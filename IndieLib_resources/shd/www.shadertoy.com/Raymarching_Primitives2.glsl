...
vec2 map( in vec3 pos )
{
    ...
    vec2 res = opU( vec2( sdPlane(     pos), 1.0 ),
			...
			vec2( udRoundBox(  pos-vec3( 0.0,0.25, 0.0), vec3(0.15), 0.1 ), 41.0 )
);
//float RoundBox(vec2 p , vec2 BoxOrigin ,float width, float height, float sharpness)
#endif

    res = opU( res, vec2( udRoundBox(  pos-vec3( 1.0,0.25, 1.0), vec3(0.15), 0.1 ), 41.0 ) );
    res = opU( res, vec2( udRoundBox(  pos-vec3( 1.5,0.25, 1.0), vec3(0.15), 0.1 ), 41.0 ) );
    res = opU( res, vec2( udRoundBox(  pos-vec3( 1.0,0.25, 1.5), vec3(0.15), 0.1 ), 41.0 ) );
    res = opU( res, vec2( udRoundBox(  pos-vec3( 1.0,0.55, 1.0), vec3(0.15), 0.1 ), 41.0 ) );
    res = opU( res, vec2( udRoundBox(  pos-vec3( 1.5,0.55, 1.0), vec3(0.15), 0.1 ), 41.0 ) );
    res = opU( res, vec2( udRoundBox(  pos-vec3( 1.0,0.55, 1.5), vec3(0.15), 0.1 ), 41.0 ) );
...
void main( )
{
    ...
    for( int n=0; n<AA; n++ )
    {
        ...
        vec3 ro = vec3( 4.0*cos(1.1*time + 6.0*mo.x), 1.0 + 2.0*mo.y, 4.0*sin(1.1*time + 6.0*mo.x) );
