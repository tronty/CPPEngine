uniform vec2 txPacmanPos;
uniform vec2 txPacmanMovDirNex;
uniform vec2 txGhost0PosDir;
uniform vec2 txGhost1PosDir;
uniform vec2 txGhost2PosDir;
uniform vec2 txGhost3PosDir;
uniform vec2 txPoints;
uniform vec2 txState;
uniform vec2 txLives;
uniform vec2 txMode;
...
vec3 drawMap( vec3 col, in vec2 xlv_TEXCOORD0 )
{
    vec2 p = xlv_TEXCOORD0/iResolution.y;
...
vec3 drawPacman( vec3 col, in vec2 xlv_TEXCOORD0, in vec4 pacmanPos, in vec3 pacmanMovDirNex )
{
...
vec3 drawGhost( vec3 col, in vec2 xlv_TEXCOORD0, in vec3 pos, in float dir, in float id, in vec3 mode )
{
...
vec3 drawScore( in vec3 col, in vec2 xlv_TEXCOORD0, vec2 score, float lives )
{
    // score
    vec2 p = xlv_TEXCOORD0/iResolution.y;
    // lives
...
void main( )
{
    ...
    // CRT
    //------------------------
    vec3 col = ToSrgb( Tri(xlv_TEXCOORD0.xy/iResolution.xy)*Mask(xlv_TEXCOORD0.xy) );
    //col = texture2D( iChannel1, xlv_TEXCOORD0.xy/iResolution.xy ).xyz;
    ...
    // map
    col = drawMap( col, xlv_TEXCOORD0 );

    // pacman
    col = drawPacman( col, xlv_TEXCOORD0, pacmanPos, pacmanDir );

    // ghosts
    for( int i=0; i<4; i++ )
        col = drawGhost( col, xlv_TEXCOORD0, ghostPos[i].xyz, ghostPos[i].w, float(i), mode );
    
    // score
    col = drawScore( col, xlv_TEXCOORD0, points, lives );
