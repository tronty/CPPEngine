...
float mapGround(vec3 pos){
    vec3 te = texture2D(iChannel0, pos.xz*.1).rgb;
...
// ------------------------------------
	...
	for (int i=0;i<ANTIALIASING;i++) {
    	...
    	// camera	
        q = (xlv_TEXCOORD0.xy+subPix)/iResolution.xy;
        vec2 p = -1.0+2.0*q;
