...
void main( ) {  
    if( letterBox(xlv_TEXCOORD0, iResolution.xy, 2.35) ) {
        gl_FragColor = vec4( 0., 0., 0., 1. );
    } else {
        vec4 col = texture2D(iChannel0, ivec2(xlv_TEXCOORD0));
        vec4 clouds = texture2D(iChannel1, ivec2(xlv_TEXCOORD0));
        ...
        if (intersectSphere(ro,rd,vec4(FLAG_POSITION,HUMANOID_SCALE*INV_SCENE_SCALE*2.))) {
            for(int x=0; x<AA; x++) {
                for(int y=0; y<AA; y++) {
                    vec2 p = (-iResolution.xy + 2.0*(xlv_TEXCOORD0 + vec2(x,y)/float(AA) - .5))/iResolution.y;
                    rd = cam * normalize(vec3(p,CAMERA_FL)); 
        ...
        // lens flare
        col.rgb += SUN_COLOR*lensflare(xlv_TEXCOORD0)*smoothstep(-.3,.5,dot(rd,SUN_DIR));       
        ...
        // noise
        col.rgb -= hash12(xlv_TEXCOORD0)*.025;
