...
void main( ){
    ...
    // Viewport
    vec2 uv = (2.*xlv_TEXCOORD0-iResolution.xy) / max(iResolution.x, iResolution.y); // max -1..1
    uv *=1.5;
    ...
    vec3 mArmUL = vec3(.0,.07, 40.+45.*sin(animTime));
    vec3 mArmUR = vec3(.0,.07, 40.+55.*sin(animTime-.9*PI));
    
    vec3 mLegUL = vec3(.0,.07, 45.*sin(animTime-.9*PI));
    vec3 mLegLL = vec3(.0,.07, 45.*clamp(sin(animTime-73./8.*PI),-.0,1.));
    vec3 mShoeUL = vec3(.0,.0, -45.*clamp(sin(animTime-.9*PI),-1.,.0));
    
    vec3 mLegUR = vec3(.0,.07, 45.*sin(animTime));
