...
void main( ) {
	...
	vec2 offset = iMouse.xy;
		//
	float xpart = xlv_TEXCOORD0.x/resolution.x;
	float ypart = xlv_TEXCOORD0.y/resolution.y;
   
	//
	float clip = 410.0;
	float ypartClip = xlv_TEXCOORD0.y/clip;
	float ypartClippedFalloff = clamp(2.0-ypartClip,0.0,1.0);
    //如果下面加上绝对值这里就需要减去一个值
	float ypartClipped = min(ypartClip,1.0)-0.2;
	float ypartClippedn = 1.0-ypartClipped;
	//
    ypartClippedn =1.0015;
	float xfuel = 1.0-(2.0*xpart-1.0);//pow(1.0-abs(2.0*xpart-1.0),0.5);
    xfuel = pow(1.0-abs(2.0*xpart-1.0),0.5);
    //xfuel=1.0;
	//
	float timeSpeed = 0.5;
	float realTime = timeSpeed*time;
	//
	vec2 coordScaled = 0.01*xlv_TEXCOORD0 - 0.02*vec2(offset.x,0.0);
	...
	//在这里加上abs绝对值就能使之上下对应
	float flames = pow(abs(ypartClipped),0.3*xfuel)*pow(noise,0.3*xfuel);
	//
	...
	vec2 sparkCoord = xlv_TEXCOORD0 - vec2(2.0*offset.x,190.0*realTime);
