...
VsGenVelOutput VS_GENVELOCITY( VsGenVelInput input )
{
    ...
    output.Pos = gridPos;
    output.Velocity = (gridPos - prevGridPos).xyz * 0.5f * gridDim * recTimeStep * velocityMultiplier;
                              ...
                              inout TriangleStream<GsGenVelOutput> triStream )
{
    ...
    for(int j=0; j<2; j++)
    {
        output.Pos = float4(intersections[j].Pos, 0, 1);
        output.Velocity = intersections[j].Velocity;
        triStream.Append( output );

        output.Pos = float4((intersections[j].Pos + (normal * projSpacePixDim)), 0, 1);
        output.Velocity = intersections[j].Velocity;
        triStream.Append( output );
    }
    triStream.RestartStrip( );
...
float4 PS_RESOLVE( GsResOutput input ) : COLOR0
{
    if( stencilbufferTex2D.Load(int3(input.Tex.x, input.Tex.y,0)).g )
        return OBSTACLE_INTERIOR;
    return OBSTACLE_EXTERIOR;
}
...
PsGenVelOutput PS_GENVELOCITY( GsGenVelOutput input )
{
    ...
    output.Obstacle = OBSTACLE_BOUNDARY;
    return output;
}
