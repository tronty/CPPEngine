...
float4 PS_ADVECT_MACCORMACK( GS_OUTPUT_FLUIDSIM input ) : COLOR0
{
...
float4 PS_ADVECT( GS_OUTPUT_FLUIDSIM input ) : COLOR0
{
    if( IsOutsideSimulationDomain(input.texcoords.xyz ) )
        return 0;
...
float4 PS_VORTICITY( GS_OUTPUT_FLUIDSIM input ) : COLOR0
{
...
float4 PS_CONFINEMENT( GS_OUTPUT_FLUIDSIM input ) : COLOR0
{
    if( IsOutsideSimulationDomain(input.texcoords.xyz ) )
        return 0;
...
float4 PS_DIFFUSE( GS_OUTPUT_FLUIDSIM input ) : COLOR0
{
...
float PS_DIVERGENCE( GS_OUTPUT_FLUIDSIM input ) : COLOR0
{
...
float PS_SCALAR_JACOBI( GS_OUTPUT_FLUIDSIM input ) : COLOR0
{
...
float4 PS_APPLY_AIR_PRESSURE ( GS_OUTPUT_FLUIDSIM input) : COLOR0
{
...
float4 PS_PROJECT( GS_OUTPUT_FLUIDSIM input ): COLOR
{
    if( IsOutsideSimulationDomain(input.texcoords.xyz ) )
        return 0;
...
float4 PS_SIGNED_DISTANCE_TO_LIQUIDHEIGHT( GS_OUTPUT_FLUIDSIM input) : COLOR0
{
    float distance = input.cell0.z - liquidHeight;    
    return distance;
}

float4 PS_INJECT_LIQUID( GS_OUTPUT_FLUIDSIM input) : COLOR0
{
...
float4 PS_REDISTANCING(  GS_OUTPUT_FLUIDSIM input ) : COLOR0 
{
    const float dt = 0.1f;
...
float4 PS_EXTRAPOLATE_VELOCITY( GS_OUTPUT_FLUIDSIM input ) : COLOR0
{
...
float4 PS_LIQUID_STREAM( GS_OUTPUT_FLUIDSIM input ) : COLOR0 
{
    if( IsNonEmptyCell(input.texcoords.xyz) )
        return 0;
...
float4 PS_APPLY_GRAVITY( GS_OUTPUT_FLUIDSIM input) : COLOR0
{
    if( IsOutsideSimulationDomain(input.texcoords.xyz ) )
        return 0;
...
float4 PS_GAUSSIAN( GS_OUTPUT_FLUIDSIM input ) : COLOR0 
{
    if( IsNonEmptyCell(input.texcoords.xyz) )
        return 0;
...
float4 PS_COPY_TEXURE( GS_OUTPUT_FLUIDSIM input ) : COLOR0 
{
    if( IsNonEmptyCell(input.texcoords.xyz) )
        return 0;
...
float4 PS_ADD_DERIVATIVE_VEL( GS_OUTPUT_FLUIDSIM input ) : COLOR0 
{
    if( IsNonEmptyCell(input.texcoords.xyz) )
        return 0;
...
PSDrawBoxOut PS_STATIC_OBSTACLE( GS_OUTPUT_FLUIDSIM input ) : COLOR0
{
    PSDrawBoxOut voxel;
    voxel.obstacle = OBSTACLE_BOUNDARY;
    voxel.velocity = 0;
...
float4 PS_DRAW_TEXTURE( VS_OUTPUT_FLUIDSIM input ) : COLOR0
{
