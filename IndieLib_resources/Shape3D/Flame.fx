...
float3 noiseScale = float3( 1.0, 1.0, 1.0 );
float3 noiseAnim = float3( 0.0, -0.1, 0.0 );

float4 flameColor = float4( 0.2, 0.2, 0.2, 1.0 );
float3 flameScale = float3( 1.0, -1.0, 1.0 );
float3 flameTrans = float3( 0.0, 0.0, 0.0 );
...
vertexOutput flameVS(VsIn IN)
{
    ...
    OUT.position = mul(objPos, worldViewProj);
...
float4 flamePS(vertexOutput IN) : COLOR
{
	...
	//return float4(IN.NoisePos,1);
	//return float4(IN.FlamePos,1);
//	return tex3D(NoiseMap,IN.NoisePos) * flameColor;
//  return turbulence(IN.NoisePos) * flameColor;

	float2 uv;
	uv.x = length(IN.FlamePos.xz);	// radial distance in XZ plane
	uv.y = IN.FlamePos.y;
	
	////uv.y += turbulence(IN.NoisePos) * noiseStrength;
	//uv.y += turbulence(IN.NoisePos) * noiseStrength / uv.x;
	uv.y += turbulence(IN.position) * noiseStrength / uv.x;

	float4 c = tex2D(tExplosion, IN.uv) * flameColor;
	c = tex2D(tExplosion, IN.uv);
	c.a=1;
	return c;
}
