...
vec4 main(VsOut In) : COLOR
{
	...
	{
		rgb = HSLtoRGB(vec3(169, 255, (192 + Uint8(turbulence2(In.texCoord, 64)) / 4)));
	}
	else if(iI==4)
	{
	...
	else if(iI==7)
	{
		...
		float yValue = (In.texCoord.y - noiseHeight / 2) / float(noiseHeight) + turbPower * turbulence(vec2(h - In.texCoord.y, w - In.texCoord.x) /* , turbSize */ ) / 256.0;
  ...
  // get a random offset
  //float r = .01 * random( vec3( 12.9898, 78.233, 151.7182 ), 0.0, In.position.xyz );
  r = f;
