...
VertexToPixel CelVertexShader(VS_INPUT input)
{
    VertexToPixel output;
 
    // Transform the position
    float4 worldPosition = mul(float4(input.Pos, 1), World);
    float4 viewPosition = mul(worldPosition, View);
    output.Position = mul(viewPosition, Projection);
 
    // Transform the normal
    output.Normal = normalize(mul(float4(input.Normal, 1), WorldInverseTranspose)).xyz;
...
VertexToPixel OutlineVertexShader(VS_INPUT input)
{
    VertexToPixel output = (VertexToPixel)0;
 
    // Calculate where the vertex ought to be.  This line is equivalent
    // to the transformations in the CelVertexShader.
    float4 original = mul(mul(mul(float4(input.Pos, 1), World), View), Projection);
