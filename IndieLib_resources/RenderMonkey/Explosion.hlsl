...
float flamability = float( 0.34 );
float pressure = float( 0.60 );
float powerBoost = float( 0.16 );
float intensity = float( 1.00 );
float speed = float( 0.15 );
float noisiness = float( 0.50 );
float time_0_X;
float explositivity = float( 0.45 );
sampler2D Flame;
//sampler Noise;
...
float4 main(float2 texCoord: TEXCOORD0) : COLOR 
{
   ...
   float n = noise(float3(noisiness * texCoord, flamability * time_0_X - pressure * dist));

   float4 flame = tex2D(Flame, float2(size * powerBoost + size * (2 * n - dist), 0));
