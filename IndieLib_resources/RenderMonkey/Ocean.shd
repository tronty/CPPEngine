...
float4 scale = float4( 0.01, 0.01, 0.01, 1.00 );
...
VS_OUTPUT main3(VS_INPUT IN){
   ...
   //Pos.z = -30;
	...
	float4 v = float4(IN.Pos, 1);
	Out.Pos = mul(worldViewProj, v);

   //Out.Pos = mul(worldViewProj, Pos);
   Out.pos = IN.Pos.xyz * scale;
   Out.vVec = IN.Pos - view_position;
...
float waveSpeed = float( 0.34 );
float noiseSpeed = float( 0.18 );
float fadeBias = float( 0.30 );
float fadeExp = float( 6.08 );
float time_0_X;
float4 waterColor = float4( 0.20, 0.35, 0.68, 1.00 );
