...
float skyZBias = float( -48.00 );
float4 view_position = float4( 0.0, 0.0, 0.0, 0.0 );
float skyScale = float( 20.00 );
...
VS_OUTPUT main3(VS_INPUT IN){
   VS_OUTPUT Out;

   // Get the sky in place
   IN.Pos.y += skyZBias;
   //Out.Pos = mul(worldViewProj, float4(IN.Pos.xyz * skyScale + view_position, 1));
   Out.Pos = mul(worldViewProj, float4(IN.Pos.xyz * skyScale + view_position, 1));
   // Pass position to the fragment shader
   Out.texCoord = IN.Pos.xyz * scale;
...
float noiseScale = float( 4.10 );
float noiseBias = float( -1.00 );
float4 lightDir = float4( 0.92, 0.28, 0.27, 0.00 );
float4 sunColor = float4( 1.00, 0.82, 0.44, 1.00 );
float sunFallOff = float( 3.50 );
float sunSharpness = float( 0.42 );
float cloudSpeed = float( 0.05 );
float noiseSpeed = float( 0.02 );
float time_0_X;
