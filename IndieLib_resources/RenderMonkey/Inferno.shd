...
float time_0_X;
float sideFade = float( 18.60 );
float sideFadeSharpness = float( 0.05 );
float wobbleScale = float( 0.03 );
float burnSpeed = float( 0.57 );
float randomnessRate = float( 0.24 );
float yFade = float( 0.60 );
float xScale = float( 1.50 );
float yScale = float( 0.60 );
...
float4 main(float2 texCoord: TEXCOORD0) : COLOR 
{
   ...
   float4 flame = tex2D(Flame, float2(heat, 0));
