...
VolumeVertex PerlinFireVS( appdata IN )
{
    ...
    __ClipPos = mul( vec4( _Pos.x, _Pos.y, _Pos.z, 1 ), WorldViewProj );

    __RayDir = _Pos - EyePos;
    __Pos = _Pos;   // supposed to have range -0.5 ... 0.5
...
{
	//return vec4(0, 1, 0, 1);
    vec3 Dir = normalize(_RayDir) * StepSize;
    float Offset = bJitter ? texture2D(JitterTexture, _ClipPos.xy / 256.0 ).r : 0;

    // Jitter initial position
    vec3 Pos = _Pos + Dir * Offset;

    vec3 resultColor = 0;
    float SceneZ = texture2D( ScreenDepth, _ClipPos.xy );

    for(int i=0; i<nSamples; i++)
    {
        //vec3 Pos = _Pos + Dir * Offset + Dir * i;
...
{
    vec4 ClipPos = vec4( input.Pos.x, _Pos.y, _Pos.z, 1.0 );
...
vec4 SimplePS( MeshVertex In ) : COLOR
{
    ...
    vec3 lightCol = saturate( dot( lightDir, _Normal ) );
    vec4 shadeColor = vec4( 1.0, 1.0, 1.0, 1.0 );
    vec4 realPosLight = vec4( _Pos - LightPos, 1.0 );
  
    float maxCoord = max( abs( realPosLight.x ), max( abs( realPosLight.y ), abs( realPosLight.z ) ) );
    
    vec4 diffuseColor = texture2D (SceneTexture, _TexCoord);
    ...
    float shadow = 1.0;// ??? projectedDepth - 0.001f > texture2D(ShadowMap, _Pos - LightPos ).r ? 1 : 0;
	...
	//return vec4(1,0,0,1);    
    return (diffuseColor * vec4( lightCol, 1.0 ) * shadeColor * LightIntensity * pow( saturate( 150.0 / length( LightPos - _Pos ) ), 3.0 ));
}
*/
