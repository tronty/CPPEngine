...
*/
[Vertex shader]
ROW_MAJOR mat4 matWorldViewProj MVPSEMANTIC;   
ROW_MAJOR mat4 matWorld;   
vec4 vecLightDir;
vec4 vecEye;
...
VS_OUTPUT main(VS_INPUT IN)
{
	VS_OUTPUT Out = (VS_OUTPUT)0;
      Out.Pos = mul(vec4(IN.Position, 1), matWorldViewProj); // transform Position
      // Create tangent space to get normal and light to the same space.
      float3x3 worldToTangentSpace;
      worldToTangentSpace[0] = mul(normalize(vec4(IN.Tangent,1)), matWorld).xyz;
      worldToTangentSpace[1] = mul(normalize(vec4(IN.BiNormal,1)), matWorld).xyz;
      worldToTangentSpace[2] = mul(normalize(vec4(IN.Normal,1)), matWorld).xyz;
      // Just pass textures trough
      Out.Tex = IN.uv;
      vec4 PosWorld = mul(vec4(IN.Position, 1), matWorld);
...
vec4 main(VS_OUTPUT IN) : COLOR
{
      ...
      vec3 N = (2.0 * (texture2D(NormalMapSampler, IN.Tex)))-1.0;
      // diffuse
      float D = saturate(dot(N, IN.Light));
      // reflection
      vec3 R = normalize(2 * D * N-IN.Light);
      // specular
