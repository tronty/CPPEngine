...
VsOut main(VsIn In){
  ...
  Out.noise = 10.0 *  (-.10) * turbulence( .5 * In.normal );
  // get a 3d noise using the position, low frequency
  float b = 5.0 * pnoise( 0.05 * In.position, float3( 100.0 ) );
  // compose both noises
  float displacement = (- 10.) * Out.noise + b;
...
float4 main(VsOut IN): COLOR {
  ...
  float4 color = float4(float3( IN.uv * ( 1. - 2. * IN.noise ), 0.0 ), 1.0 );
  return color;
}
