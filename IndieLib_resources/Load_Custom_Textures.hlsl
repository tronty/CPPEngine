...
#define RGB(r,g,b) pow(float3(float(r), float(g), float(b))/255.0, float3(2.22))
...
float3 Render(in float2 uv)
{
	...
	float SunRay = pow(tex2D(iChannel1, float2(0.1*t, atan(SunC.x,SunC.y))).xyz, float3(2.22)).x;
	...
	float V = 1.0-(atan(length(Normal.xz), Normal.y)) / Pi;
 	float3 Ground = pow(tex2D(iChannel0, float2(U-t/80.0, V)).xyz, float3(2.22));
	float3 Cloud  = pow(tex2D(iChannel1, float2(U-t/75.0, V)).xyz, float3(2.22));
	float3 Cloud2 = pow(tex2D(iChannel1, float2(U-t/75.0+0.001, V)).xyz, float3(2.22));
	float3 KsMap  = pow(tex2D(iChannel1, float2( -t/200.0, 0.8)).xyz, float3(2.22));
	float3 Night  = pow(tex2D(iChannel2, float2(U-t/80.0, V)).xyz, float3(2.22));
    ...
    float3 Light = mix(float3(1.0), RGB(255, 150, 40), Extinct);
 
    Color = Shadow*(Ground + 0.8*Sea*RGB(19,35,60));
    Color = mix(Color, float3(1.0), 2.0*Cloud);
    Color *= Light*Diffuse;
    ...
    Color *= mix(float3(1.0), RGB(255-58,255-72,255-90), 1.0*Scatter);
...
float4 main(VsOut IN): COLOR 
{
	...
	float2 uv = (2.0*IN.xy - iResolution.xy) / iResolution.y;
    float3 Color = pow(Render(1.05*uv),  float3(0.45));
