...
// Texture samplers
//--------------------------------------------------------------------------------------
sampler2D g_txMeshTexture;          // Color texture for mesh
sampler2D g_txParticleColor;        // Particle color buffer
...
VS_PARTICLEOUTPUT RenderParticlesVS( VS_PARTICLEINPUT input )
{
    ...
    // Standard transform
    float4 ip=float4(input.Position.x,input.Position.y,input.Position.z,1);
    Output.Position = mul(ip, g_mWorldViewProjection);
    Output.TextureUVI.xy = input.TextureUV; 
    Output.Color = input.Color;
    
    // Get the world position
    float3 WorldPos = mul( ip, g_mWorld ).xyz;
    ...
    float fRot = -input.fLifefRot.y;
    Output.SinCosThetaLife.x = sin( fRot );
    Output.SinCosThetaLife.y = cos( fRot );
    Output.SinCosThetaLife.z = input.fLifefRot.x;
...
//--------------------------------------------------------------------------------------
VS_SCREENOUTPUT CompositeParticlesVS( float3 Position : POSITION )
{
    VS_SCREENOUTPUT Output;

    Output.Position = float4(Position.x,Position.y,Position.z,1);
...
{ 
	...
	float4 diffuse = tex2D(g_txMeshTexture, input.TextureUVI.xy );
	...
	// Fade
	float alpha = diffuse.a * (1.0 - input.SinCosThetaLife.z);
...
// Composite partices into the scene
//--------------------------------------------------------------------------------------
float4 CompositeParticlesPS( VS_SCREENOUTPUT input ) : COLOR
{
	//return float4(0, 1, 0, 1);
	// Load the particle normal data, opacity, and color
	float3 loadpos = float3(input.Position.xy,0);
    float4 particlebuffer = tex2D(g_txMeshTexture, loadpos.xy );
    float4 particlecolor = tex2D(g_txParticleColor, loadpos.xy );
...
VS_MESHOUTPUT MeshVS( VS_MESHINPUT input )
{
    ...
    float3 ip=float4(input.Position.x,input.Position.y,input.Position.z,1);
    Output.Position = mul( ip, g_mWorldViewProjection );
    Output.wPos = mul( ip, g_mWorld );
...
VS_MESHOUTPUT MeshInstVS( VS_MESHINPUT input, uint instanceID : SV_INSTANCEID )
{
	...
	float4 wPos = mul( float4(input.Position.x,input.Position.y,input.Position.z,1), g_mWorldInst[instanceID] );
...
float4 MeshPS( VS_MESHOUTPUT input ) : COLOR
{
	//return float4(0, 1, 0, 1);
...
technique10 RenderParticlesToBuffer
{
    pass P0
    {
		...
		SetBlendState( DeferredBlending, float4( 0.0, 0.0, 0.0, 0.0 ), 0xFFFFFFFF );
...
technique10 RenderParticles
{
    pass P0
    {
		...
		SetBlendState( ForwardBlending, float4( 0.0, 0.0, 0.0, 0.0 ), 0xFFFFFFFF );
        SetDepthStencilState( DepthRead, 0 );
        SetRasterizerState( RSSolid );
    }
}
...
technique10 CompositeParticlesToScene
{
    pass P0
    {
		...
		SetBlendState( CompositeBlending, float4( 0.0, 0.0, 0.0, 0.0 ), 0xFFFFFFFF );
