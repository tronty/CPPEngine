...
{
	VS_OUTPUT1 OUT;
	__hposition=mul(vec4(_position, 1), worldViewProj);
    	__uv=_uv;
    	return OUT;
}
...
vec4 main1( VS_OUTPUT1 IN ) : COLOR0
{
	vec4 np4=noise(_hposition.xyz);
	np4=texture2D(noiseSampler, _uv);
	vec4 texcol = np4 * texture2D(colorSampler, _uv.x);
	texcol.a=1;
...
vec4 main2( VS_OUTPUT2 IN ) : COLOR0
{
	vec4 np4=noise(_hposition.xyz);
	np4=texture2D(noiseSampler, _uv);
	vec4 texcol = np4 * texture2D(colorSampler, _uv.x);
...
vec4 main( VS_OUTPUT1 IN ) : COLOR0
{
  vec3 v_texCoord3D=_hposition.xyz;
  ...
  else
  {
  	...
  	return vec4(0.5 + 0.85 * vec3(n, n, n), 1.0);  
  }
