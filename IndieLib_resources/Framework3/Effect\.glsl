...
{
	VS_OUTPUT1 OUT;
	OUT.hposition=mul(vec4(IN.position, 1), worldViewProj);
    	OUT.uv=IN.uv;
    	return OUT;
}
...
{
	vec4 np4=noise(IN.hposition.xyz);
	np4=texture2D(noiseSampler, IN.uv);
	vec4 texcol = np4 * texture2D(colorSampler, IN.uv.x);
	texcol.a=1;
...
{
  vec3 v_texCoord3D=IN.hposition.xyz;
