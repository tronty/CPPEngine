...
float4 main1( VS_OUTPUT1 IN ) : COLOR0
{
	float4 np4=noise(IN.hposition.xyz);
	np4=tex2D(noiseSampler, IN.uv);
	float4 texcol = np4 * tex2D(colorSampler, IN.uv.x);
	texcol.a=1;
...
float4 main2( VS_OUTPUT2 IN ) : COLOR0
{
	float4 np4=noise(IN.hposition.xyz);
	np4=tex2D(noiseSampler, IN.uv);
	float4 texcol = np4 * tex2D(colorSampler, IN.uv.x);
