...
{
	VsOut Out=(VsOut)0;
    vec4 noisePos = mul(worldViewProj, vec4(In.Position, 1.0));
    float i = (noise(noisePos.xyz) + 1.0) * 0.5;
    Out.Color = vec3(i, i, i);
#if 1
  // add time to the noise parameters so it's animated
  Out.noise = 10.0 *  (-.10) * turbulence( .5 * In.Normal + time );
  float b = 5.0 * pnoise( 0.05 * In.Position + vec3( 2.0 * time ), vec3( 100.0 ) );
  float displacement = (- Out.noise) + b;

  vec3 newPosition = In.Position + In.Normal * displacement;
  Out.position = mul(worldViewProj, vec4(newPosition, 1.0));
#else
    // displacement along normal
    vec4 position = vec4(In.Position, 1.0) + (vec4(In.Normal, 1.0) * i * Displacement);
    position.w = 1.0;
...
vec4 main(VsOut IN): COLOR {
	return vec4(IN.Color, 1.0)*texture2D(tColor, IN.UV);
}
