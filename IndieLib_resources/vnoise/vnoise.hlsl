...
{
	VsOut Out=(VsOut)0;
    float4 noisePos = mul(worldViewProj, float4(In.Position, 1.0));
    float i = (noise(noisePos.xyz) + 1.0) * 0.5;
    Out.Color = float3(i, i, i);
#if 1
  ...
  Out.position = mul(worldViewProj, float4(newPosition, 1.0));
    ...
    float4 position = float4(In.Position, 1.0) + (float4(In.Normal, 1.0) * i * Displacement);
    position.w = 1.0;
...
float4 main(VsOut IN): COLOR {
	return float4(IN.Color, 1.0)*tex2D(tColor, IN.UV);
}
