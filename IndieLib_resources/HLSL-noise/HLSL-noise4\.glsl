...
ROW_MAJOR mat4 modelViewProjection MVPSEMANTIC;
VS_OUTPUT mainVS( const VS_INPUT IN)
{
    VS_OUTPUT OUT;

	OUT.hposition = mul( vec4(IN.position, 1), modelViewProjection );

	OUT.v_texCoord2D = IN.texture0.xy;

	OUT.v_texCoord3D = IN.position.xyz;

	OUT.color = vec4(0,1,0,1);//IN.color;
...
VS_OUTPUT mainVVS( VS_INPUT IN )
{
    ...
    vec4 noisePos = mul(TextureMatrix/*[0]*/ , vec4(IN.position,1));

    float i = (ssnoise(noisePos.xyz/*, pg*/) + 1.0) * 0.5;
    OUT.color = vec4(i, i, i, 1.0);

    // displacement along normal
    vec4 position = vec4(IN.position,1) + (vec4(IN.normal, 1.0) * i * Displacement);
    //vec4 position = IN.position + (IN.normal * i * Displacement);
    position.w = 1.0;
...
vec4 mainPS( VS_OUTPUT IN ) : COLOR
{
  ...
  //float n = noise(vec3(4.0 * IN.v_texCoord3D.xyz * (2.0 + sin(0.5 * time))));
//if(NoiseTechnique==1)
  n = ssnoise(vec3(2.0 * IN.v_texCoord3D.xyz * noiseScale * (2.0 + sin(0.5 * time))));
/*else //if(NoiseTechnique==4)
  n = ssnoise(vec3(2.0 * IN.v_texCoord3D.xyz * (2.0 + sin(0.5 * time))));
  */
  //float n = noise(vec4(8.0 * IN.v_texCoord3D.xyz, 0.5 * time));
  //float n = snoise(vec4(4.0 * IN.v_texCoord3D.xyz, 0.5 * time));

  vec4 color = IN.color * vec4(0.5 + 0.5 * vec3(n, n, n), 1.0);
...
vec4 fragmentShaderPNoise(VS_OUTPUT IN) : COLOR
{
  ...
  IN.v_texCoord3D=vec3(IN.v_texCoord2D, 0);

  float n = ssnoise(vec3(4.0 * IN.v_texCoord3D.xyz));
...
vec4 fragmentShaderMarble(VS_OUTPUT IN) : COLOR
{
  ...
  vec3 diffuse=lerp(color1, color2, cos(IN.v_texCoord3D.z*0.1+6*turbulence(5,IN.v_texCoord3D.xyz,2,0.6)));
  vec4 color = vec4(diffuse, 1.0);
	return GammaCorrect4(color);
}
vec4 fragmentShaderFur(VS_OUTPUT IN) : COLOR
{
  ...
  vec3 diffuse=lerp(color1, color2, clamp(0.7*turbulence(6,IN.v_texCoord3D.xyz,3,0.9), 0,1));
...
vec4 fragmentShaderCloud(VS_OUTPUT IN) : COLOR
{
  ...
  vec3 diffuse=lerp(color1, color2, cos(IN.v_texCoord3D.z*0.5+2*turbulence(6,IN.v_texCoord3D.xyz,2,0.5))*0.9);
  vec4 color = vec4(diffuse, 1.0);
	return GammaCorrect4(color);
}
...
//	return inoise(vec3(IN.texcoord, 0.0))*0.5+0.5;
}
*/
