...
float snoise(float3 v)
{
	...
	float3 x2 = x0 - i2 + C.yyy; // 1.0*C.x = 1/3 = C.y
	float3 x3 = x0 - D.yyy;      // -5.0+3.0*C.x = -0.5 = -D.y
...
{
	float f;//???=length(position - ExpPosition) - Radius;
	return f;
}
...
float4 main(VsOut IN): COLOR 
{
	...
	float2 p = (fragCoord.xy / resolution.xy) * 2.0 - 1.0;
