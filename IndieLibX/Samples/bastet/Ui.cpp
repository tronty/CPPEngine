...
namespace Bastet{
  ...
  BorderedWindow::BorderedWindow(int height, int width, int y, int x){
    if(y==-1 || x==-1){
      ...
      screen_h=STX_Service::GetWindowInstance()->Getwidth();
      screen_w=STX_Service::GetWindowInstance()->Getheight();
      if(y==-1)
	y=(screen_h-height-2)/2-1;
      if(x==-1)
	x=(screen_w-width-2)/2-1;
    }
    _border=new WINDOW(height+2,width+2,y,x);
    _window=new WINDOW(_border,height,width,1,1); 
  ...
  void Ui::MessageDialog(const std::string &message){
    ...
    while(ch!=' ' && ch!=13); //13=return key!=STX_KEY_ENTER, it seems
  }
