...
namespace Bastet{
  int Evaluate(const Well *w, int extralines){
    ...
    //lines
    int score=100000000*extralines;
  ...
  boost::array<int,nBlockTypes> BastetBlockChooser::ComputeMainScores(const Well *well, BlockType currentBlock){
  ...
  BlockType BastetBlockChooser::GetNext(const Well *well, const Queue &q){
    boost::array<int,nBlockTypes> mainScores=ComputeMainScores(well,q.front());
    boost::array<int,nBlockTypes> finalScores=mainScores;
    ...
    boost::array<int,nBlockTypes> temp(finalScores);
  ...
  BlockType NoPreviewBlockChooser::GetNext(const Well *well, const Queue &q){
    assert(q.empty());
    boost::array<int,nBlockTypes> finalScores;
