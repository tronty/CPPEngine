...
Grid::Grid() : IRenderer::GetRendererInstance()(0),IRenderer::GetRendererInstance()(0), m_layout(0), m_renderQuadBuffer(0),
...
int Grid::Initialize( int gridWidth, int gridHeight, int gridDepth,stx_Effect::stx_Technique* technique )
{
    int hr(0);
...
int Grid::CreateVertexBuffers( stx_Effect::stx_Technique* technique )
{
    int hr(0);
	...
	m_layout = IRenderer::GetRendererInstance()->addVertexFormat(layoutDesc, elementsOf(layoutDesc), -1);
    ...
    (CreateVertexBuffer(sizeof(VS_INPUT_FLUIDSIM_STRUCT)*m_numVerticesRenderQuad,
        BIND_VERTEX_BUFFER, &m_renderQuadBuffer, renderQuad, m_numVerticesRenderQuad));
    ...
    (CreateVertexBuffer(sizeof(VS_INPUT_FLUIDSIM_STRUCT)*m_numVerticesSlices,
        BIND_VERTEX_BUFFER, &m_slicesBuffer, slices , m_numVerticesSlices));
    ...
    (CreateVertexBuffer(sizeof(VS_INPUT_FLUIDSIM_STRUCT)*m_numVerticesBoundarySlices,
        BIND_VERTEX_BUFFER, &m_boundarySlicesBuffer, boundarySlices, m_numVerticesBoundarySlices));
    ...
    (CreateVertexBuffer(sizeof(VS_INPUT_FLUIDSIM_STRUCT)*m_numVerticesBoundaryLines,
        BIND_VERTEX_BUFFER, &m_boundaryLinesBuffer, boundaryLines, m_numVerticesBoundaryLines));
...
void Grid::DrawSlices( void )
{
    ...
    //DrawPrimitive( PRIM_3_CONTROL_POINT_PATCHLIST, m_layout, &m_slicesBuffer,stride, offset, 0, m_numVerticesSlices );
                         ...
                         unsigned int VertexCount )
{
    ...
    IRenderer::GetRendererInstance()->setVertexFormat(0,  layout );
...
int Grid::CreateLayout( FormatDesc* layoutDesc, unsigned int numElements,
                           stx_Effect::stx_Technique* technique, VertexFormatID   layout)
{
    int hr(0);
