...
bool App::onKey(const uint key, const bool pressed){
	if (D3DApp::onKey(key, pressed)) return true;
...
bool App::load(){
	...
	if (d3d->CheckDeviceFormat(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, D3DFMT_X8R8G8B8, 0, D3DRTYPE_SURFACE, (D3DFORMAT) MAKEFOURCC('R','2','V','B')) == D3D_OK){
		...
		if (d3d->CheckDeviceFormat(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, D3DFMT_X8R8G8B8, 0, D3DRTYPE_TEXTURE, D3DFMT_A16B16G16R16F) == D3D_OK){
			if (caps.DeclTypes & D3DDTCAPS_FLOAT16_4) supportsHalf = true;
		}
		...
		if (caps.VertexShaderVersion >= D3DVS_VERSION(3,0)){
			...
			if (d3d->CheckDeviceFormat(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, D3DFMT_X8R8G8B8, D3DUSAGE_QUERY_VERTEXTEXTURE, D3DRTYPE_TEXTURE, D3DFMT_A16B16G16R16F) == D3D_OK){
				if (caps.DeclTypes & D3DDTCAPS_FLOAT16_4) supportsHalf = true;
			} else if (d3d->CheckDeviceFormat(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, D3DFMT_X8R8G8B8, D3DUSAGE_QUERY_VERTEXTEXTURE, D3DRTYPE_TEXTURE, D3DFMT_A32B32G32R32F) != D3D_OK){
				ErrorMsg("This demo requires R2VB or VTF support.");
				return false;
			}
	...
	if (caps.PixelShaderVersion < D3DPS_VERSION(2,0)){
		ErrorMsg("This demo requires pixel shader 2.0.");
		return false;
	}
