...
D3DXFROMWINEVECTOR3 g_vecChopper = D3DXFROMWINEVECTOR3(0.0f, 0.5f, 0.0f);
...
bool DoInit()
{
  ...
  g_ChopperMesh.Load("/AdvAniCh12/chopper.X", eSimpleShader, true, true);
  //g_ChopperMesh.Load("/AdvAniCh12/heli.x", eSimpleShader, false, true);
  g_RotorMesh.Load("/AdvAniCh12/Rotor.X", eSimpleShader, true, true);
  g_ShadowMesh.Load("/AdvAniCh12/Shadow.X", eSimpleShader, true, true);
...
//D3DXFROMWINEMatrixLookAtLH(D3DXFROMWINEMATRIX *pout, CONST D3DXFROMWINEVECTOR3 *peye, CONST D3DXFROMWINEVECTOR3 *pat, CONST D3DXFROMWINEVECTOR3 *pup);
  ...
  IRenderer::GetRendererInstance()->BeginScene();{
    ...
    D3DXFROMWINEMatrixRotationYawPitchRoll(&R, g_rotChopper, 0.0f, -0.0872222f);
    //matWorld._41 = g_vecChopper.x*0.0f;  // Set position manually in matrix
    //matWorld._42 = g_vecChopper.y*0.01f;
    //matWorld._43 = g_vecChopper.z*0.0f;

    if(1){
		float s=0.25f;
		D3DXFROMWINEMatrixScaling(&S, s, s, s);
		D3DXFROMWINEVECTOR3 t(g_vecChopper.x*0.0f, g_vecChopper.y*1.0f, g_vecChopper.z*0.0f);
	...
	{
static D3DXFROMWINEVECTOR3 g_vecChopper_ = D3DXFROMWINEVECTOR3(0.0f, 0.5f, 0.0f);
	static __DWORD__ LastTime = timeGetTime();
  __DWORD__ Time = timeGetTime();
  __DWORD__ Elapsed = Time - LastTime;
  LastTime = Time;

  g_vecChopper_.x += (float)cos(g_rotChopper) * ((float)Elapsed / 3.0f);
  g_vecChopper_.z -= (float)sin(g_rotChopper) * ((float)Elapsed / 3.0f);
		float s=0.25f;
		D3DXFROMWINEMatrixScaling(&S, s, s, s);
		D3DXFROMWINEVECTOR3 t(g_vecChopper_.x*1.0f, 0.5f, g_vecChopper_.z*1.0f);
    		D3DXFROMWINEMatrixTranslation( &T, t.x, t.y, t.z);
		matWorld=R*S*T;
    g_ChopperMesh.render(&matWorld);
	}}
