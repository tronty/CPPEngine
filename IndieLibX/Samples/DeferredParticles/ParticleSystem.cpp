...
void CopyParticlesToVertexBuffer( PARTICLE_VERTEX* pVB, D3DXFROMWINEVECTOR3 vEye, D3DXFROMWINEVECTOR3 vRight, D3DXFROMWINEVECTOR3 vUp )
{
    ...
    for( int i = g_NumUsedParticles - 1; i >= 0; i-- )
    {
        ...
        float fRot = g_pParticleArray[index].fLifefRot.y;
        float fFade = g_pParticleArray[index].fFade;
        D3DXFROMWINEVECTOR4 vColor = g_pParticleArray[index].Color;
...
//--------------------------------------------------------------------------------------
INT CParticleSystem::CreateParticleSystem( UINT NumParticles )
{
...
void CParticleSystem::Init()
{
    for( UINT i = 0; i < m_NumParticles; i++ )
    {
        ...
        m_pParticles[i].fLifefRot.x = m_fStartTime;
        m_pParticles[i].fFade = 0.0f;

        m_pParticles[i].fLifefRot.y = RPercent() * 3.14159f * 2.0f;

        float fLerp = RPercent();
        m_pParticles[i].Color = m_vColor0 * fLerp + m_vColor1 * ( 1.0f - fLerp );
/*
                                     ...
                                     D3DXFROMWINEVECTOR3 vWindVel, D3DXFROMWINEVECTOR3 vGravity )
{
    if( m_fCurrentTime > 0 )
    {
        for( UINT i = 0; i < m_NumParticles; i++ )
        {
            float t = m_pParticles[i].fLifefRot.x / m_fLifeSpan;
            float tm1 = t - 1.0f;
            ...
            m_pParticles[i].fLifefRot += D3DXFROMWINEVECTOR2(fElapsedTime, fRot);
    ...
    else
    {
        for( UINT i = 0; i < m_NumParticles; i++ )
        {
            ...
            m_pParticles[i].fLifefRot += D3DXFROMWINEVECTOR2(fElapsedTime, 0.0f);
        }
    }
...
void CStalkParticleSystem::Init()
{
    for( UINT i = 0; i < m_NumParticles; i++ )
    {
        ...
        m_pParticles[i].fLifefRot = D3DXFROMWINEVECTOR2(m_fStartTime, RPercent() * 3.14159f * 2.0f);
    ...
    for( UINT s = 0; s < m_NumStreamers; s++ )
    {
        ...
        for( UINT i = 0; i < ParticlesPerStream; i++ )
        {
            if( index < m_NumParticles )
            {
                ...
                m_pParticles[index].fLifefRot = D3DXFROMWINEVECTOR2(m_fStartTime, RPercent() * 3.14159f * 2.0f);
                m_pParticles[index].fFade = 0.0f;

                m_pParticles[index].fRotRate = RPercent() * 1.5f;

                float fLerp = RPercent();
                m_pParticles[index].Color = ( m_vColor0 * fLerp ) + ( m_vColor1 * ( 1.0f - fLerp ) );
/*
                                                ...
                                                D3DXFROMWINEVECTOR3 vWindVel, D3DXFROMWINEVECTOR3 vGravity )
{
    if( m_fCurrentTime > 0 )
    {
        for( UINT i = 0; i < m_NumParticles; i++ )
        {
            ...
            m_pParticles[i].fLifefRot.x += fElapsedTime;
            m_pParticles[i].fFade = fFade;

            m_pParticles[i].fLifefRot.y += fRot;
