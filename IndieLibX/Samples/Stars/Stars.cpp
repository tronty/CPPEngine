...
D3DXFROMWINEVECTOR3 GetPosition3(float varphi,float theta,float r=SKY_RADIUS)
{
	...
	position=m_pSkyEmitter->CalculatePosition(varphi, theta, JD);
#else
...
void DrawBorders()
{
	...
	for(unsigned int i=0;i<1565;i++)
	{
		...
		#else
		D3DXFROMWINEVECTOR3 pos1=GetPosition3(constellationBorders[i].ra, constellationBorders[i].dec);
		D3DXFROMWINEVECTOR3 pos2=GetPosition3(constellationBorders[i+1].ra, constellationBorders[i+1].dec);
		#endif
		#if 0
