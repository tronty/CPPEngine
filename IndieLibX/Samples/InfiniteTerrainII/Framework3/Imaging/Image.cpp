...
Image::Image(const Image &img){
	...
	stx_memcpy(pixels, img.pixels, size);

	nExtraData = img.nExtraData;
	extraData = new unsigned char[nExtraData];
	stx_memcpy(extraData, img.extraData, nExtraData);
}
	...
	if (nMipMaps != actualMipMaps){
		...
		if (arraySize > 1){
			...
			for (int i = 0; i < arraySize; i++){
				stx_memcpy(newPixels + i * size, pixels + i * oldSize, firstMipSize);
			}
...
bool Image::removeMipMaps(const int firstMipMap, int mipMapsToSave){
	...
	stx_memcpy(newPixels, getPixels(firstMipMap), size);
