...
uint *rearrange(Array <ObjMaterial> &materials, const uint *srcIndices, const uint nIndices){
	...
	for (uint i = 0; i < materials.getCount(); i++){
		...
		for (uint j = 0; j < mInds->getCount(); j += 2){
			stx_memcpy(indices + currIndex, srcIndices + (*mInds)[j], (*mInds)[j + 1] * sizeof(uint));
			currIndex += (*mInds)[j + 1];
		}
	}
...
bool Model::loadObj(const char *fileName){
	...
	if (texCoords.getCount()){
		...
		if (tangents.getCount()){
			...
			stx_memcpy(tIndices, indices, nIndices * sizeof(uint));
		...
		if (binormals.getCount()){
			...
			stx_memcpy(bIndices, indices, nIndices * sizeof(uint));
...
bool Model::loadT3d(const char *fileName, const bool removePortals, const bool removeInvisible, const bool removeTwoSided, const float texSize){
	...
	stx_memcpy(texIndices, vtxIndices, nIndices * sizeof(uint));
...
bool Model::computeNormals(const bool flat){
	...
	if (flat){
		...
		stx_memset(normals, 0, nVertices * sizeof(vec3));
		...
		uint *nrmIndices = new uint[nIndices];
		stx_memcpy(nrmIndices, indices, nIndices * sizeof(uint));
...
bool Model::computeTangentSpace(const bool flat){
	...
	if (flat){
		...
		stx_memset(tangents,  0, nVertices * sizeof(vec3));
		stx_memset(binormals, 0, nVertices * sizeof(vec3));
		stx_memset(normals,   0, nVertices * sizeof(vec3));
		...
		uint *indicesS = new uint[nIndices];
		uint *indicesT = new uint[nIndices];
		stx_memcpy(indicesS, indices, nIndices * sizeof(uint));
		stx_memcpy(indicesT, indices, nIndices * sizeof(uint));
			...
			for (uint j = 0; j < 3; j++){
				...
				for (uint k = 0; k < streams.getCount(); k++){
					if (signed(k) != vertexStream){
						...
						stx_memcpy(dest, streams[k].vertices + nComp * streams[k].indices[i + j], nComp * sizeof(float));
						dest += nComp;
					}
				}
			}
...
restart:
				...
				if (c > 0.00001f && c < 0.99999f){
					...
					if (dot(pl, pl) < 0.0001f){
						...
						for (uint x = 0; x < 3; x++){
							...
							stx_memcpy(tri.attributes[x], triangles[i].attributes[x], nAttrib * sizeof(float));
						}
	...
	for (uint i = 0; i < streams.getCount(); i++){
		...
		if (signed(i) == vertexStream){
			...
			for (uint j = 0; j < triangles.getCount(); j++){
				for (uint k = 0; k < 3; k++){
					stx_memcpy(dest, triangles[j].attributes[k] + currComp, nComp * sizeof(float));
					dest += nComp;
				}
			}
...
void Model::copyVertex(const uint destIndex, const Model &srcModel, const uint srcIndex){
	for (uint i = 0; i < streams.getCount(); i++){
		...
		stx_memcpy(dest, src, nComp * sizeof(float));
	}
}
...
bool Model::merge(const Model *model){
	if (streams.getCount() == 0){
		...
		for (uint i = 0; i < streams.getCount(); i++){
			...
			stx_memcpy(streams[i].vertices + streams[i].nVertices * streams[i].nComponents, model->streams[i].vertices, model->streams[i].nVertices * vertexSize);
...
void Model::copy(const Model *model){
	...
	for (uint i = 0; i < model->getStreamCount(); i++){
		...
		stx_memcpy(vertices, stream.vertices, stream.nComponents * stream.nVertices * sizeof(float));
		uint *indices = new uint[nIndices];
		stx_memcpy(indices, stream.indices, nIndices * sizeof(uint));
...
void Model::optimizeStream(const StreamID streamID){
	...
	for (uint i = 0; i < nVert; i++){
		...
		indexRemap[i] = index;
		stx_memcpy(vertices + index * nComp, vertex, nComp * sizeof(float));
	}
		...
		if (!hash.insert(iIndex, &index)){
			if (separateArrays){
				for (i = 0; i < nStreams; i++){
					...
					stx_memcpy(dest, streams[aStreams[i]].vertices + streams[aStreams[i]].indices[j] * nc, nc * sizeof(float));
				}
			} else {
