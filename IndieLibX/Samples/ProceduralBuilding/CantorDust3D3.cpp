...
void CantorDust(D3DXFROMWINEVECTOR3 aPosition, D3DXFROMWINEVECTOR3 aSize, unsigned int NIter)
{
	...
	{D3DXFROMWINEVECTOR3 position(aPosition.x-size.x, aPosition.y+size.y, aPosition.z-size.z);
	 CantorDust(position, size, NIter-1);}
	{D3DXFROMWINEVECTOR3 position(aPosition.x+size.x, aPosition.y+size.y, aPosition.z-size.z);
	 CantorDust(position, size, NIter-1);}
	{D3DXFROMWINEVECTOR3 position(aPosition.x-size.x, aPosition.y+size.y, aPosition.z+size.z);
	 CantorDust(position, size, NIter-1);}
...
int init(const char* aTitle)
{
	...
	CantorDust(aPosition, aSize, Niterations);
...
{
	IRenderer* r=IRenderer::GetRendererInstance("CantorDust3D3");
