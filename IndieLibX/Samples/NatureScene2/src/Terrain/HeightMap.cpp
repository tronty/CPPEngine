                      ...
                      const Image3*  watermap)
{
  ...
  //LOG_PRINT("hmBytes=%x\n", hmBytes);
  if(!wmBytes || ! hmBytes)
    { /* return */ LOG_PRINT("Null heightmap or watermap content\n");return 0;}

  //LOG_FNLN;
  //LOG_PRINT("height=%d\n", height);
  //LOG_PRINT("width=%d\n", width);
  if(!(height == HEIGHT_MAP_DEPTH) || !(width == HEIGHT_MAP_WIDTH))
    { /* return */ LOG_PRINT("Heightmap has unexpected width or height\n");return 0;}

  if(!(height == watermap->getHeight()) || !(width == watermap->getWidth()))
    { /* return */ LOG_PRINT("Watermap has unexpected width or height\n");return 0;}
  ...
  for(int y = 0; y < height; y++)
  {
    for(int x = 0; x < width; x++)
    {
      ...
      vertices[index].uv.set(float(x)/HEIGHT_MAP_WIDTH, float(y)/HEIGHT_MAP_DEPTH);
		max_.x = std::max( max_.x, vertices[index].vertex.x);
		max_.y = std::max( max_.y, vertices[index].vertex.y);
		max_.z = std::max( max_.z, vertices[index].vertex.z);
		min_.x = std::min( min_.x, vertices[index].vertex.x);
		min_.y = std::min( min_.y, vertices[index].vertex.y);
		min_.z = std::min( min_.z, vertices[index].vertex.z);
    }
  }
	LOG_PRINT("min.x=%f\n", min_.x);
	LOG_PRINT("min.y=%f\n", min_.y);
...
const float HeightMap::getInterpolatedHeight(const D3DXFROMWINEVECTOR3 &position) const
{
  ...
  x0       = clamp(int(local.x/TERRAIN_WIDTH_SCALE), 0, width  - 2);
  z0       = clamp(int(local.z/TERRAIN_DEPTH_SCALE), 0, height - 2);
...
void  HeightMap::computeBTangents()
{
  ...
  for(y = 0; y < height; y++)
  {
    for(x = 0; x < width; x++)
    {
      ...
      x2            = clamp(x + 1, 0, width  - 1);
      y1            = clamp(y - 1, 0, height - 1);
      y2            = clamp(y + 1, 0, height - 1);
...
bool  HeightMap::buildWeightMap(Image3 & image)
{
  ...
  Image3 img;
  ubyte *bytes = img.create(FORMAT_RGBA8, 256, 256, 1, 1);
...
const HMVertex     *HeightMap::getVertexStream() const { return &vertices[0]; }

const int  HeightMap::getIndexCount() const { return indices.size();  }
