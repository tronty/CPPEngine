...
{
public:
    Grid( ID3D10Device* IRenderer::GetRendererInstance() );
    virtual ~Grid( void );

    int Initialize( int gridWidth, int gridHeight, int gridDepth,stx_Technique* technique );
...
protected:
    ...
    int CreateVertexBuffers  (stx_Technique* technique);
    ...
    int CreateLayout( D3D10_INPUT_ELEMENT_DESC* layoutDesc, unsigned int numElements,stx_Technique* technique, VertexFormatID** layout);
    ...
    void DrawPrimitive( D3D10_PRIMITIVE_TOPOLOGY PrimitiveType, VertexFormatID* layout, exBufferID** vertexBuffer,unsigned int* stride, unsigned int* offset, unsigned int StartVertex, unsigned int VertexCount );
