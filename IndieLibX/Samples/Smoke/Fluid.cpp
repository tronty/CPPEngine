...
Fluid::Fluid( ) : m_pGrid(0), m_pEffect(0),
...
int Fluid::Initialize( int width, int height, int depth, FLUID_TYPE fluidType )
{
    ...
    try
    {
        m_pGrid = new Grid( IRenderer::GetRendererInstance() );
    }
    catch(...)
    {
        SAFE_DELETE(m_pGrid);
...
int Fluid::LoadShaders( )
{
    ...
    (loadTextureFromFile(IRenderer::GetRendererInstance(), "..\\..\\Media\\fireBase.dds", m_pEffect, m_effectPath, "Texture_inDensity"));
...
int Fluid::CreateTextureAndViews(int rtIndex, D3D10_TEXTURE3D_DESC TexDesc)
{
