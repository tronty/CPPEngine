...
void CreateILTexture(ubyte *data, int width, int height, int nch, TextureID& id)
{
	Image3 img;
	ubyte* data_=img.create(FORMAT_RGBA8, width, height, 1, 1);
	stx_memcpy(data_, data, width*height*nch*sizeof(ubyte));
	img.swap(0, 2);
	id=IRenderer::GetRendererInstance()->addTexture(&img, false, IRenderer::GetRendererInstance()->Getlinear());
}

void CreateLibNoiseTexture(utils::Image& image, TextureID& id)
{
	int width=image.GetWidth();
	int height=image.GetHeight();
	int nch=image.GetStride();
	ubyte *data=(ubyte *)image.GetSlabPtr();
	Image3 img;
	ubyte* data_=img.create(FORMAT_RGBA8, width, height, 1, 1);
	stx_memcpy(data_, data, width*height*nch*sizeof(ubyte));
	id=IRenderer::GetRendererInstance()->addTexture(&img, false, IRenderer::GetRendererInstance()->Getlinear());
...
LOG_FNLN;
  ...
  CreateLibNoiseTexture(destImage, textureid[0]);
	return 0;
...
LOG_FNLN;
  ...
  CreateLibNoiseTexture(destImage, textureid[1]);
...
LOG_FNLN;
  ...
  CreateLibNoiseTexture(destImage, textureid[2]);
...
LOG_FNLN;
  ...
  CreateLibNoiseTexture(destImage, textureid[3]);
...
#else
	CreateILTexture(data, width, height, 4, texture);
#endif
	delete [] data;
...
void render()
{
	...
	if(STX_Service::GetInputInstance()->IsMouseButtonPressed(STX_MBUTTON_LEFT))	 I-=;
	if(STX_Service::GetInputInstance()->IsMouseButtonPressed(STX_MBUTTON_RIGHT)) I+=;
	if(I>3) I=0;
	if(I<0) I=3;
	...
	IRenderer::GetRendererInstance()->setTexture("Base", textureid[I]);
...
{
	IRenderer* r=IRenderer::GetRendererInstance("ComplexTexture");	
