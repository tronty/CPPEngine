...
void CParticleEvent::ProcessPropEqualsValue(CMinMax<D3DXFROMWINEVECTOR3> &prop,
    std::vector<CParticleEmitterToken>::iterator &TokenIter, 
    std::vector<CParticleEmitterToken>::iterator &EndIter)
{
  // next token should be =
...
void CParticleEvent::ProcessPropEqualsValue(CMinMax<D3DXFROMWINECOLOR> &prop,
    std::vector<CParticleEmitterToken>::iterator &TokenIter, 
    std::vector<CParticleEmitterToken>::iterator &EndIter)
{
  // next token should be =
...
void CParticleEvent_Color::DoItToIt(CParticle &part)
{
  ...
  if (m_NextFadeEvent) {
    D3DXFROMWINECOLOR newcolor = static_cast<CParticleEvent_Color *>(m_NextFadeEvent)->GetColor().GetRandomNumInRange();
    float timedelta = m_NextFadeEvent->GetActualTime() - GetActualTime();
    if (timedelta == 0) timedelta = 1; // prevent divide by zero errors
...
void CParticleEvent_Velocity::DoItToIt(CParticle &part)
{
  ...
  if (m_NextFadeEvent) {
    D3DXFROMWINEVECTOR3 newvalue = static_cast<CParticleEvent_Velocity *>(m_NextFadeEvent)->GetVelocity().GetRandomNumInRange();
    float timedelta = m_NextFadeEvent->GetActualTime() - GetActualTime();
    if (timedelta == 0) timedelta = 1; // prevent divide by zero errors
