...
void DrawPlanets()
{
	   //D3DXFROMWINEMATRIX mvp=GetMVP();D3DXFROMWINEVec3ProjectArray(m_pSkyEmitter->screenpositions, stride,constellationpositions, stride, &viewport, &matProj, &matView, &matWorld, constellationnum);
	   
	for(unsigned int i=0;i<10;i++)
	{
		//D3DXFROMWINEVec3TransformCoord(&pos, &pos, &mvp);
	
		D3DXFROMWINEVECTOR3 pos(m_pSkyEmitter->screenpositions[i].x, m_pSkyEmitter->screenpositions[i].y, 0.0f);
		
		IRenderer::GetRendererInstance()->drawCircle(pos.x,pos.y,5.0f,D3DXFROMWINEVECTOR4(1,1,1,1));//,1.0f);

		#if 1
		 char buf[256];
		 stx_snprintf(buf, 256, "%s", &names[i][0]);
	  	 IRenderer::GetRendererInstance()->drawText(buf, pos.x+3.0f, pos.y,
			15, 18,
			IRenderer::GetRendererInstance()->GetdefaultFont(),
			IRenderer::GetRendererInstance()->GetlinearClamp(),
			IRenderer::GetRendererInstance()->GetblendSrcAlpha(),
...
void DrawBorders()
{
	...
	//D3DXFROMWINEVec3ProjectArray(constellationscreenpositions, stride,constellationpositions, stride, &viewport, &matProj, &matView, &matWorld, constellationnum);
