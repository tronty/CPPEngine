...
#ifdef WIN32
void set_module_handle(unsigned int hM)
{
...
void set_module_restypename(const char * restypename)
{
	...
	else
	{
		...
		resource.res_type_id = (unsigned int)restypename;
	}
}
#endif
...
char * read_text_file(const char * filename)
{
	...
	{
		path.path.push_back(".");
		path.path.push_back(MEDIA_PATH"programs");
 ...
 **/
char * read_text_file(const char * filename, const char * type, unsigned int hModule)
{
	...
	else
	{
		...
		if(hr)
		{
			...
			if(hg)
			{
				__DWORD__ sz = SizeofResource(resource.hModule, hr);
