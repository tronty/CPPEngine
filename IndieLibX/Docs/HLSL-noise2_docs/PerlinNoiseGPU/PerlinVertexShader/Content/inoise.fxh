...
float perm(float x)
{
	return tex1Dlod(permSampler, float4(x, 0, 0, 0));
}

float4 perm2d(float2 p)
{
	return tex2Dlod(permSampler2d, float4(p, 0, 0));
}

//3D
float gradperm(float x, float3 p)
{
	return dot(tex1Dlod(permGradSampler, float4(x, 0, 0, 0)), p);
}

//4D
float grad(float x, float4 p)
{
	return dot(tex1Dlod(gradSampler4d, float4(x, 0, 0, 0)), p);
}
