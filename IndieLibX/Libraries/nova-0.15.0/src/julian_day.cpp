...
void ln_get_date_from_sys (struct ln_date * date)
{
	...
	/* fill in date struct */
	date->seconds = st.wSecond + (st.wMilliseconds / 1000000);
	date->minutes = st.wMinute;
	date->hours = st.wHour;
	date->days = st.wDay;
	date->months = st.wMonth + 1;
	date->years = st.wYear + 1900;
#else
	struct tm * gmt;
	struct timeval tv;
	struct timezone tz;
...
/*! \fn void ln_date_to_zonedate (struct ln_date * date, struct ln_zonedate * zonedate, int gmtoff)
...
*/
void ln_date_to_zonedate (struct ln_date * date, struct ln_zonedate * zonedate, int gmtoff)
{
	double jd;
	struct ln_date dat;
