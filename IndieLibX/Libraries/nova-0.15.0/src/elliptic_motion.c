...
{
	double Eo = libnova_M_PI2;
	double F, M1;
	double D = libnova_M_PI4;
	int i;
	...
	F = sgn (M); 
	M = fabs (M) / (2.0 * libnova_M_PI);
	M = (M - (int)M) * 2.0 * libnova_M_PI * F;
	
	if (M < 0)
		M = M + 2.0 * libnova_M_PI;
	F = 1.0;
...
double ln_get_ell_orbit_len (struct ln_ell_orbit * orbit)
{
	...
	return libnova_M_PI * (( 21.0 * A - 2.0 * G - 3.0 * G) / 8.0);
}
