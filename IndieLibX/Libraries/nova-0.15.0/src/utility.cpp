...
#define DM_PI (2*libnova_M_PI)
#define RADIAN (180.0 / libnova_M_PI)
...
double ln_hms_to_rad (struct ln_hms *hms)
{
    ...
    radians = ((double)hms->hours / 24.0) * 2.0 * libnova_M_PI;
    radians += ((double)hms->minutes / 60.0) * 2.0 * libnova_M_PI / 24.0;
    radians += ((double)hms->seconds / 60.0) * 2.0 * libnova_M_PI / 1440.0;
...
double ln_dms_to_rad (struct ln_dms *dms)
{
    ...
    radians =  fabs((double)dms->degrees / 360.0 * 2.0 * libnova_M_PI);
    radians += fabs((double)dms->minutes / 21600.0 * 2.0 * libnova_M_PI);
    radians += fabs((double)dms->seconds / 1296000.0 * 2.0 * libnova_M_PI);
...
double ln_range_radians (double angle)
{
    ...
    if (angle >= 0.0 && angle < (2.0 * libnova_M_PI))
    	return angle;
    
	temp = (int)(angle / (libnova_M_PI * 2.0));

	if (angle < 0.0)
		temp --;
	temp *= (libnova_M_PI * 2.0);
	return angle - temp;
}
...
double ln_range_radians2 (double angle)
{
    ...
    if (angle > (-2.0 * libnova_M_PI) && angle < (2.0 * libnova_M_PI))
    	return angle;
...
* \brief human readable int/lat position to double int/lat position
* \ingroup conversion
*/
...
* \brief double int/lat position to human readable int/lat position
* \ingroup conversion
*/
