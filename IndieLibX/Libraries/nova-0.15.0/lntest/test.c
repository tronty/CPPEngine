...
int test_result (char * test, double calc, double expect, double tolerance)
{
	...
	if (diff) {
		STX_PRINT ("[FAILED]\n");
		STX_PRINT ("	Expected %8.8f but calculated %8.8f %0.12f error.\n\n", expect, calc, diff);
		return 1;
	} else {
		STX_PRINT ("[PASSED]\n");
		STX_PRINT ("	Expected and calculated %8.8f.\n\n", calc);
		return 0;
	}
}
...
int lunar_test ()
{
	...
	STX_PRINT ("lunar x %f  y %f  z %f\n",moon.X, moon.Y, moon.Z);
	ln_get_lunar_ecl_coords (JD, &ecl, 0);
	STX_PRINT ("lunar int %f  lat %f\n",ecl.lng, ecl.lat);
	ln_get_lunar_equ_coords_prec (JD, &equ, 0);
	STX_PRINT ("lunar RA %f  Dec %f\n",equ.ra, equ.dec);
	STX_PRINT ("lunar distance km %f\n", ln_get_lunar_earth_dist(JD));
	STX_PRINT ("lunar disk %f\n", ln_get_lunar_disk(JD));
	STX_PRINT ("lunar phase %f\n", ln_get_lunar_phase(JD));
	STX_PRINT ("lunar bright limb %f\n", ln_get_lunar_bright_limb(JD));
	return failed;
}
...
int main ()
{
	...
	STX_PRINT ("Test completed: %d tests, %d errors.\n", test_number, failed);
