...
namespace Arbaro
{
  ...
  : tree(tree), transf(trf), offset(offs), par(tree->params), 
    lpar(par.level[min_(stlev,3)]), stemlevel(stlev)
  {
    splitErrorValue = 0;
    substemErrorValue = 0;
    leavesErrorValue = 0;
  ...
  void Stem::make() {
	  ...
	  if (stemlevel==0) {
		  //float baseWidth = max_(baseRadius,stemRadius(0));
  ...
  void Stem::pruning() {
	  ...
	  while (segm >= 0 && length > 0.001*par.scale_tree) {
		  ...
		  // get new length
		  float minlen = length/2; // shorten max_. half of length
		  float maxlen = length-origlen/15; // shorten min_ of 1/15 of orig. len
		  length = min_(max_(segmentLength*segm,minlen),maxlen);
  ...
  float Stem::stemBaseRadius() {
	  if (stemlevel == 0) { // trunk
		  ...
		  // max_ radius is the radius of the parent at offset
  ...
  float Stem::stemRadius(float h) {
	  ...
	  float Z = min_(h/length,1.0f); // min_, to avoid rounding errors
  ...
  void Stem::prepareSubstemParams() {
	  //int level = min_(stemlevel+1,3);
	  Level lpar_1 = par.level[min_(stemlevel+1,3)];
