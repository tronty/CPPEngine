...
namespace opentree
{
  ...
  public:
    ...
    inline static void split(otWeberStem* splitstem, otMatrix& trf, float s_angle, int nseg, bool prune=false)
    {
      ...
      split_angle = max_(0.0f, split_angle - declination);
    ...
    static inline float stemRadius(const otWeberStem* stem, float h)
    {
      ...
      float Z = min_(h / stem->length, 1.0f); // limited to one.
      ...
      else if (level.taper >= 1 && level.taper <= 3)
      {
        ...
        if (level.taper < 2 && Z3 >= taper_z)
          ...
          radius_z = (1-depth) * taper_z + depth * sqrtf(max_(0.0f,powf(taper_z, 2) - powf(Z3 - taper_z, 2)));
      }
      
      // Flaring
      if (level.levelNumber == 0)
      {
        float y = max_(1 - 8 * Z, 0.0f); // "the value of y is limited to a minimum of zero"
