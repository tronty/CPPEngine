...
public:
  ...
  void setupArray(const OTL_Vertex* vertices, const unsigned int numvertices);//, const Tangent* tangents = 0);
  void setupVBOArray(const unsigned int vertices);
  unsigned int createVBO(const OTL_Vertex* vertices, const unsigned int count);
  ...
  OTL_Texture* createTexture(unsigned x, unsigned y);
  OTL_Texture* loadTexture(const char* filename);
  void setTexture(OTL_Texture* tex, unsigned int layer=0);
