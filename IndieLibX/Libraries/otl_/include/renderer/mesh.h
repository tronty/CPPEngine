...
public:
  ...
  void createTangents()
  {
    ...
    for (size_t a = 0; a < indexlist->getCount(); a++)
    {
      ...
      const OTL_Vertex& v1 = vertexlist->getVertices()[triangle.v1];
      const OTL_Vertex& v2 = vertexlist->getVertices()[triangle.v2];
      const OTL_Vertex& v3 = vertexlist->getVertices()[triangle.v3];
  ...
  void draw(Renderer* renderer)
  {
    ...
    renderer->setupArray(vertexlist->getVertices(), vertexlist->getCount());//, (tangentlist)?tangentlist->getTangents():0);
#if 0
  LOG_FNLN;
  LOG_PRINT("vertexlist->getVertices()=%x\n", vertexlist->getVertices());
  LOG_PRINT("vertexlist->getCount()=%d\n", vertexlist->getCount());
  LOG_PRINT("indexlist->getTriangles()=%x\n", indexlist->getTriangles());
  LOG_PRINT("indexlist->getCount()=%d\n", indexlist->getCount());
#endif
