  ...
  public:
    BBCSimple(otTree* tree, int rot_groups) : rot_groups(rot_groups)
    {
      ...
      for (int i=0; i<max_groups; i++)
      {
        ...
        if (n > 0)
        {
          //LOG_PRINT(" between %.2f and %.2f", rot_group[i].dist_min.Y, rot_group[i].dist_max.Y);
          filled_groups++;
        }
      ...
      LOG_PRINT("\n");
      LOG_PRINT("Total count of %d leaves\n", total);
      LOG_PRINT("Filled groups: %d of %d\n", filled_groups, max_groups);
    ...
    unsigned int getVertices(otVertices& vertices)
    {
      for (int i=0; i<max_groups; i++)
      {
        ...
        for (int jj=0; jj<rot_groups; jj++)
        {
          ...
          //LOG_PRINT("v(%d,%d): <%.1f,%.1f,%.1f><%.1f,%.1f,%.1f><%.1f,%.1f,%.1f><%.1f,%.1f,%.1f>\n",
