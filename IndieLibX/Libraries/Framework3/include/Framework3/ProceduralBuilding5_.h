...
struct IndexedMesh
{
    ...
    std::vector<stx_VertexPositionColor> g_vertices_;
	std::vector<__WORD__> g_indices_;
	...
	void init(std::string afn="", std::string afiletosave="", bool abUseLua=true)
	{
		...
		if((m_IndexedMesh.m_vertices.size()==0)&&(g_vertices_.size()==0))
			stx_exit(0);
		{
		shd = IRenderer::GetRendererInstance()->addShaderFromFile("/Framework3/plainSHD2C.hlsl", "main", "main");
		FormatDesc format[] =
		{
...
void DrawVertex(IndexedMesh* aIndexedMesh, const D3DXFROMWINEMATRIX& aW, const D3DXFROMWINEVECTOR3& aC, const D3DXFROMWINEVECTOR3& aV_)
{
	g_color3_=aC;
	g_color4_=D3DXFROMWINEVECTOR4(aC.x, aC.y, aC.z, 1.0f);
	aIndexedMesh->m_mW.push_back(aW);
    stx_VertexPositionColor vx_(aV_, g_color4_);
	aIndexedMesh->m_vertices.push_back(vx_);
	aIndexedMesh->m_indices.push_back(aIndexedMesh->m_vertices.size()-1);

	stx_VertexPositionColor vx=aIndexedMesh->m_vertices[aIndexedMesh->m_vertices.size()-1];
	g_vertices_.push_back(vx);g_indices_.push_back(g_vertices_.size()-1);
}
		...
		const D3DXFROMWINEVECTOR2 aY=D3DXFROMWINEVECTOR2(-1.0f, 1.0f))
{
	...
	for(unsigned int i=iStart;i<iStop;i++)
	{
		...
		m.m_mW.push_back(I);
		m.m_colors4.push_back(m_IndexedMesh.m_colors4[i]);
	...
	if(rp<0)
		return;
			...
			for(unsigned int j=i1;j<i2;j++)
			{
				...
				D3DXFROMWINEVec3TransformCoord(&m_IndexedMesh.m_vertices[j].Position, &m_IndexedMesh.m_vertices[j].Position, &m);
				IndexedMesh_.m_mW.push_back(I);
				IndexedMesh_.m_vertices.push_back(m_IndexedMesh.m_vertices[j]);
			}
			D3DXFROMWINEVECTOR3 ac;
		...
		D3DXFROMWINEVECTOR3& amax)
{
	...
	while(i<v.size())
	{
		...
		{
			D3DXFROMWINEVECTOR3 vC_=D3DXFROMWINEVECTOR3(atof(v[i+2].c_str()), atof(v[i+3].c_str()), atof(v[i+4].c_str()));  /*
			if(vC_!=vC)
			{
				LOG_FNLN;
			} */

            D3DXFROMWINEVECTOR3 red(1.0f, 0.0f, 0.0f);
            if(red==vC_)
                Nred++;
...
if(m_bUseLua){
	...
	for(unsigned int i=0;i<m_IndexedMesh.m_vertices.size();i++)
		D3DXFROMWINEVec3TransformCoord(&m_IndexedMesh.m_vertices[i].Position, &m_IndexedMesh.m_vertices[i].Position, &m_IndexedMesh.m_mW[i]);
...

{
	...
	{
		for( unsigned int a = 0; a < g_vertices_.size();++a)
		{
			D3DXFROMWINEVECTOR3 pc =g_vertices_[a].Position;
	...
	{
		for( unsigned int j = 0; j < g_vertices_.size();++j)
		{
			D3DXFROMWINEVec3TransformCoord(&g_vertices_[j].Position, &g_vertices_[j].Position, &m_mWorld);
		}
	}
}
...
void render()
{
	...
	{	
		...
		IRenderer::GetRendererInstance()->setShaderConstant4x4f("worldViewProj", W);
        	IRenderer::GetRendererInstance()->DrawIndexedPrimitiveUP(PRIM_TRIANGLES, 0, g_vertices_.size(), g_indices_.size()/3, &g_indices_[0], &g_indices_[0], CONSTANT_INDEX2, &g_vertices_[0], &g_vertices_[0], sizeof(stx_VertexPositionColor));
	}
#endif
	#if 1
