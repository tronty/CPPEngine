...
void CCamera::CheckForMovement(IInput* Input)
{
	...
	if(Input->OnKeyPress(STX_KEY_PAGEUP)  || Input->OnKeyPress(STX_KEY_W) ) {
		// Move our camera forward by a positive SPEED
		MoveCamera(kSpeed);
	}
	// Check if we hit the Down arrow or the 's' key
	if(Input->OnKeyPress(STX_KEY_PAGEDOWN)  || Input->OnKeyPress(STX_KEY_S) ) {
		// Move our camera backward by a negative SPEED
		MoveCamera(-kSpeed);
	}
	// Check if we hit the Left arrow or the 'a' key
	if(Input->OnKeyPress(STX_KEY_LEFT) || Input->OnKeyPress(STX_KEY_A)) {
		// Strafe the camera left
		StrafeCamera(-kSpeed);
	}
	// Check if we hit the Right arrow or the 'd' key
	if(Input->OnKeyPress(STX_KEY_RIGHT) || Input->OnKeyPress(STX_KEY_D)) {
