...
#if 0//defined(GLES3_2)
#define glCompressedTexImage3DARB
#define glMapBufferARB
#define glUnmapBufferARB
#define GL_WRITE_ONLY_ARB 0x0000
#define GL_MIN_EXT 0x0000
#define GL_MAX_EXT 0x0000
#define GL_TEXTURE_3D_ARB 0x0000
#define GL_TEXTURE_WRAP_R_ARB 0x0000
#define GL_TEXTURE_MAX_ANISOTROPY_EXT 0x0000
#define GL_DEPTH_COMPONENT24_ARB 0x0000
#define GL_DEPTH24_STENCIL8_ARB 0x0000
#define GL_UNSIGNED_INT_24_8_ARB 0x0000
#define GL_DEPTH_STENCIL_ARB 0x0000
#endif
...
#define GL_COORD_REPLACE_ARB              0x8862
   ...
   checkGlError("");
			...
			m_pLockedData = (BYTE *)glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY_ARB);
   checkGlError("");
		#endif
...
else
{
   ...
   checkGlError("");
	...
	if (glTarget == GL_TEXTURE_3D_ARB) {
	LOG_PRINT_X("glTexParameteri:\n");
	glTexParameteri(glTarget, GL_TEXTURE_WRAP_R_ARB, ss.wrapR);
   	checkGlError("");
	}
	#endif
	LOG_PRINT_X("glTexParameteri:\n");
...
void RendererGL_1_1::setTextureSlice(const char *textureName, const TextureID texture, const int slice)
{
	...
	currentDepthWriteEnable = true;
		...
		void RendererGL_1_1::resetVrtl(const unsigned int flags)
		{
			reset0(flags);
			if (flags & RESET_TEX){
		for (unsigned int i = 0; i < MAX_TEXTUREUNIT; i++){
   ...
   checkGlError("");
				...
				else if (img->is3D())
				{
					if (isCompressedFormat(format))
					{
						...
						glCompressedTexImage3DARB(tex.glTarget, mipMapLevel, internalFormat, img->getWidth(mipMapLevel), img->getHeight(mipMapLevel), img->getDepth(mipMapLevel), 0, img->getMipMappedSize(mipMapLevel, 1), (BYTE *)(src));
   checkGlError("");
#endif
					}
					else
			...
			else
			{
				...
				if (isDepthFormat(format))
				{
					if (isStencilFormat(format))
					{
						...
						srcFormat = GL_DEPTH_STENCIL_ARB;
						#else
						srcFormat = 0;
						#endif
					}
...
BYTE *RendererGL_1_1::lockVertexBuffer(const VertexBufferID id){LOG_FNLN_X;
		return 0;}
BYTE *RendererGL_1_1::lockIndexBuffer(const IndexBufferID id){LOG_FNLN_X;
		return 0;}
BYTE *RendererGL_1_1::unlockTextureVrtl(const TextureID id,int level){return 0;}
BYTE *RendererGL_1_1::unlockVertexBuffer(const VertexBufferID id){LOG_FNLN_X;
		return 0;}
BYTE *RendererGL_1_1::unlockIndexBuffer(const IndexBufferID id){LOG_FNLN_X;
		return 0;}
...
unsigned int RendererGL_1_1::drawArrays(const Primitives primitives, const int firstVertex, const int nVertices){
	if(!glPrim[primitives]) return 0;
			apply();
...
unsigned int RendererGL_1_1::drawElements(const Primitives primitives, const int firstIndex, const int nIndices, const int firstVertex, const int nVertices){
	if(!glPrim[primitives]) return 0;
			apply();
