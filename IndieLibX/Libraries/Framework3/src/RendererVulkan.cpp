...
#include <Framework3/RendererGLDS.h>
		...
		bool samplerCompVKstd(const XSampler & s0, const XSampler &s1)
		{
       ...
       return false;
		...
		bool constantCompVKstd(const ConstantShaderGLSLGL3 & s0, const ConstantShaderGLSLGL3 &s1)
		{
       if(!(s0.name.c_str()&& s1.name.c_str()))return false;
       int i=strcmp(s0.name.c_str(), s1.name.c_str());
       if(i>0)
...
ShaderID RendererVulkan::addGLSLShader(
const char *vsText0, 
//const char *gsText0, 
const char *fsText0, 
//const char *csText0, const char *hsText0, const char *dsText0,
const char *vsName, 
//const char *gsMain, 
const char *psName, 
//const char *csMain, const char *hsMain, const char *dsMain,
                                            	const unsigned int flags)
		{
    const char* header=0;
	std::string vsText="";
	...
	char line[16];
	GLint vsResult, fsResult, linkResult;//, gsResult, csResult, hsResult, dsResult;
	char infoLog[2048];
...
shaderGL1_1.uniforms[nUniforms].location = glGetUniformLocation(shaderGL1_1.program, name);
			...
			if(shaderGL1_1.samplers.size()>1) std::sort(shaderGL1_1.samplers.begin(), shaderGL1_1.samplers.end(), samplerCompVKstd);
			if(shaderGL1_1.uniforms.size()>1) std::sort(shaderGL1_1.uniforms.begin(), shaderGL1_1.uniforms.end(), constantCompVKstd);
			...
			shadersGL.push_back(shaderGL1_1);
			return shaders.size()-1;
				}
			}
			return SHADER_NONE;
