...
bool GUIText::loadXMLSettings(XMLElement *element)
{
  ...
  if(attr = element->getAttributeByName("string"))
  {
  	LOG_FNLN;
  	LOG_PRINT("attr->getValuec()=%s\n", attr->getValuec());
  	//raise(SIGABRT);
    setString(attr->getValuec());
  }

  if(attr = element->getAttributeByName("fontIndex"))
    setFontIndex(attr->getValuei());

  if(attr = element->getAttributeByName("hScale"))
    setHeightScale(attr->getValuef());

  if(attr = element->getAttributeByName("wScale"))
    setWidthScale(attr->getValuef());
