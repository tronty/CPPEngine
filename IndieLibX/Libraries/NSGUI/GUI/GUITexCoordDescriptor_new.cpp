...
void GUITexCoordDescriptor::setType(const std::string &type)
{
  ...
  //LOG_PRINT_NONE("Unknow GUITexCoordDescriptor type -> ") + type);
  widgetType = UNKNOWN;
}
...
void GUITexCoordDescriptor::loadXMLSettings(XMLElement *element)
{
  ...
  if(attr = element->getAttributeByName("xStart"))
    xStart = attr->getValuei();

  if(attr = element->getAttributeByName("yStart"))
    yStart = attr->getValuei();

  if(attr = element->getAttributeByName("xEnd"))
    xEnd = attr->getValuei();

  if(attr = element->getAttributeByName("yEnd"))
    yEnd = attr->getValuei();

  if(attr = element->getAttributeByName("type"))
    if(widgetType = attr->getValuei());
    else
