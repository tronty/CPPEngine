...

{
...

  {
...

    {
      ...
      stx_memcpy(data, copy.data, byteCount);
...
*/
  ...
  ifstream fileInputStream(xmlFilePath, ifstream::in | ifstream::binary);
  ...
  stx_memset(stream, 0, bufferSize + 1);
...

  {
    ...
    LOG_PRINT_NONE("Failed to parse the XML File at <%s>\n",xmlFilePath);
...

{
...

  {
    ...
    stx_memset(subStream,      0, bytesCount + 1);

    stx_memcpy(subStream, (*stream), bytesCount);
...

  {
    ...
    stx_memset(subStream,      0, charCounter + 1);

    stx_memcpy(subStream, (*stream), charCounter);
