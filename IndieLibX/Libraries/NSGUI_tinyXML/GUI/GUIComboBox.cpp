...
bool GUIComboBox::loadXMLSettings(XMLElement *element)
{
  ...
  if(attr = element->getAttributeByName("lowerPanelColorScale"))
    lowerPanelColorScale = attr->getValuef();

  if(attr = element->getAttributeByName("displayScrollingRectangle"))
    displayScrollingRectangle = (attr->getValue() == "true");

  if(attr = element->getAttributeByName("hScale"))
    fontScales.y = attr->getValuef();

  if(attr = element->getAttributeByName("wScale"))
    fontScales.x = attr->getValuef();
  ...
  for(size_t i = 0; i < element->getChildrenCount(); i++)
  {
    ...
    if(childName == "Item")
    if(attr    = child->getAttributeByName("string"))
      addItem(attr->getValue());
  }

  for(size_t i = 0; i < element->getAttributeCount(); i++)
  {
    if(!(attr = element->getAttribute(i)))
      continue;

    const std::string &attrName = attr->getName();
    
    if(attrName == "fontIndex")
      fontIndex =  clampNS(attr->getValuei(), 0, 50);
  }
