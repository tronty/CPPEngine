...
GUIAlphaElement::GUIAlphaElement(std::string cbs) :  GUIRectangle(cbs)
{
...
bool GUIAlphaElement::loadXMLSettings(XMLElement *element)
{
  ...
  if(attr = element->getAttributeByName("alphaFadeScale"))
    setAlphaFadeScale(attr->getValuef());

  if(attr = element->getAttributeByName("minAlpha"))
    setMinAlpha(attr->getValuef());
...
std::string GUIAlphaElement::getLabelString()
{
