...
//LOG_PRINT_NONE("xmlFile=%s\n", xmlFile);
  ...
  {LOG_PRINT("Invalid XML GUI description file");return false;}
	LOG_FNLN;
	XMLElement* p=cfgStack.getChildByName("Panel");
	LOG_PRINT("Panel=%x\n", p);
	//if(!p)return true; // ???
...
bool GUIPanel::loadXMLSettings(XMLElement *element)
{
...
#if 1
	LOG_FNLN;
  if(attr = element->getAttributeByName("description"))
  {
    return  loadXMLSettings(attr->getValue());
    }
#endif
  ...
  for(size_t i = 0; i < element->getAttributeCount(); i++)
  {
    ...
    if(attrName == "layout")
    {
      const std::string& type = attr->getValue();
  ...
  for(size_t i = 0; i < element->getChildrenCount(); i++)
  {
    ...
    LOG_FNLN;
    LOG_PRINT("interval.x=%f, interval.y=%f\n", interval.x, interval.y);
      continue;
    }
    if(childName == "Panel")
    {
