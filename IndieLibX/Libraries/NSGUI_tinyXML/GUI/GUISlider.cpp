...
bool GUISlider::loadXMLSettings(XMLElement *element)
{
  ...
  if(attr = element->getAttributeByName("orientation"))
    orientation = (attr->getValue() == "VERTICAL") ? VERTICAL : HORIZONTAL;

  if(attr = element->getAttributeByName("progress"))
    setProgress(attr->getValuef());

  if(attr = element->getAttributeByName("discWidth"))
    discDimensions.x = attr->getValuei();

  if(attr = element->getAttributeByName("discHeight"))
    discDimensions.y = attr->getValuei();
