...
GUIFrame::~GUIFrame()
{
  clear();
	...
	TiXmlElement* pElement=rootPanelElement;
    	...
    	LOG_PRINT("children.size()=%d\n", children.size());
    	for(unsigned int i=0;i<children.size();i++)
    		LOG_PRINT("children[%d]->attributes.size()=%d\n", i, children[i]->attributes.size());
    	#endif
    	#if 0
    ...
    XMLTree::XMLTree(TiXmlElement* pElement) : NamedObject()
    {
    	...
    	{
    		LOG_PRINT("child->Name()=%s\n", child->Value());
    		XMLElement* c=new XMLElement(child);
    		for( TiXmlAttribute* attr = (TiXmlAttribute*) child->FirstAttribute(); attr; attr = (TiXmlAttribute*) attr->Next() )
    		{
    			LOG_PRINT("attr->Name()=%s, attr->Value()=%s\n", attr->Name(), attr->Value());
    			c->addAttribute(attr->Name(), attr->Value());
    		}
    	...
    	for(unsigned int i=0;i<children.size();i++)
    	{
    		XMLElement* child=children[i]->getChild(i);
    		if(child)
    		{
    		LOG_PRINT("children[%d].name=%s\n", i, children[i]->getChild(i)->getName().c_str());
    		for(unsigned int j=0;j<child->attributes.size();j++)
    		{
    			XMLAttribute* a=children[i]->getAttribute(j);
    			if(a)
    			LOG_PRINT("\tattribute[%d]=%s, %s\n", j, a->getName().c_str(), a->getValue().c_str());
    		}}
    	}
