...
if(b)
{
	...
	if(_IRenderer_BodyElement){
		...
		{
		m_bDebug=true;
		STX_PRINT("changeVertexFormat(%d)\n", vertexFormat);
#ifdef __STX_glDrawElements__
	STX_FNLN;
#endif	
	...
	if(m_bDebug)
	{
		STX_FNLN;
		STX_PRINT("changeVertexFormat(%d)\n", vertexFormat);
		STX_PRINT("vertexFormat(%x)\n", vertexFormat);
		STX_PRINT("currentVertexFormat(%x)\n", currentVertexFormat);
	}}
		...
		STX_FNLN;
if(1){
	if(m_bDebug){STX_PRINT("glEnableClientState(GL_VERTEX_ARRAY)\n");}
	}
				}
				if (!sel.vertex.size &&  curr.vertex.size)
				{
				...
				checkGlError("");
	if(1){
	if(m_bDebug){STX_PRINT("glDisableClientState(GL_VERTEX_ARRAY)\n");}
	}
				}
				...
				checkGlError("");if(1){
	if(m_bDebug){STX_PRINT("glEnableClientState(GL_NORMAL_ARRAY)\n");}
	}
				}
				if (!sel.normal.size &&  curr.normal.size) {glDisableClientState(GL_NORMAL_ARRAY);
				checkGlError("");if(1){
	if(m_bDebug){STX_PRINT("glDisableClientState(GL_NORMAL_ARRAY)\n");}
	}
}
				...
				checkGlError("");
	if(1){
	if(m_bDebug){STX_PRINT("glEnableVertexAttribArrayARB(%d)\n", i);}
	}}
					if (!sel.generic[i].size &&  curr.generic[i].size) {glDisableVertexAttribArrayARB(i);
				checkGlError("");
	if(1){
	if(m_bDebug){STX_PRINT("glDisableVertexAttribArrayARB(%d)\n", i);}
	}}
				}
	...
	if(1){
	if(m_bDebug){STX_PRINT("glClientActiveTexture(%d)\n", i);}
	}
	...
	if(1){
	if(m_bDebug){STX_PRINT("glEnableClientState(GL_TEXTURE_COORD_ARRAY)\n");}
	}
						}
						else
						{
	...
	if(1){
	if(m_bDebug){STX_PRINT("glDisableClientState(GL_TEXTURE_COORD_ARRAY)\n");}
	}
