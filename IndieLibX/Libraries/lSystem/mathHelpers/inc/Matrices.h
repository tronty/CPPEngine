...
public:
    ...
    const float* get() const;
    const float* getTranspose() const;                        // return transposed matrix
    float        getDeterminant() const;
...
private:
    ...
    float m[16];
    mutable float tm[16];                                       // transpose m. Manipulated in getTranspose for internal state caching.
};
...
inline const float* Matrix4::getTranspose() const
{
