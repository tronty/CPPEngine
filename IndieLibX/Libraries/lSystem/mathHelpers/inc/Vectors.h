...
struct Vector3
{
	...
	Vector3(Point3Df p1, Point3Df p2) : x(p2.X() - p1.X()), y(p2.Y() - p1.Y()), z(p2.Z() - p1.Z()) {};
	...
	Vector3&    rotate(float angle, const Vector3& axis);
...
inline float Vector3::angle(const Vector3& other) const
{
	...
	float cosTheta = dot/(this->magnitude() * other.magnitude());
	cosTheta = cosTheta < -1 ? -1: cosTheta;
	cosTheta = cosTheta > 1 ? 1 : cosTheta;
	return acos(cosTheta);
}

inline bool Vector3::isParallel(const Vector3& other) const
{
	return (angle(other) == 0);
}

inline Vector3& Vector3::rotate(float angle, const Vector3& axis)
{
	if (this->isParallel(axis))
		return *this;

	Vector3 axisNormalized = axis;
	axisNormalized.normalize();
	float along = this->dot(axisNormalized);
	
	Vector3 alongComponent = axisNormalized * along;
	Vector3 normalComponent = *this - alongComponent;
	Vector3 inPlane = -normalComponent.cross(axisNormalized);
	inPlane.normalize();
	
	Vector3 rotated = normalComponent * cos(angle) + inPlane * (normalComponent.magnitude() * sin(angle));
	rotated += alongComponent;
	*this = rotated;
	return *this;
}
