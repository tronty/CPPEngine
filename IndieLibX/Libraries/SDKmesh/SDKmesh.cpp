...
//--------------------------------------------------------------------------------------
HRESULT CDXUTSDKMesh::CreateFromFile( IDirect3DDevice9* pDev9,
                                      LPCTSTR szFileName,
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTSDKMesh::Create( LPCTSTR szFileName, bool bCreateAdjacencyIndices,
                              SDKMESH_CALLBACKS10* pLoaderCallbacks )
{
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTSDKMesh::Create( BYTE* pData, UINT DataBytes, bool bCreateAdjacencyIndices,
...
void CDXUTSDKMesh::TransformMesh( D3DXFROMWINEMATRIX* pWorld, double fTime )
{
    ...
    if( FTT_RELATIVE == m_pAnimationHeader->FrameTransformType )
    {
        ...
        {
            D3DXFROMWINEMatrixInverse( &mInvBindPose, NULL, &m_pBindPoseFrameMatrices[i] );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTSDKMesh::CreateAdjacencyIndices(  float fEpsilon, BYTE* pBufferData )
{
    HRESULT hr = S_OK;
    UINT IBIndex = 0;
    UINT VBIndex = 0;
    ...
    for( UINT i = 0; i < m_pMeshHeader->NumMeshes; i++ )
    {
        ...
        UINT Options = 0;
        ID3DXFROMWINE10Mesh* pMesh = NULL;

        if( DXGI_FORMAT_R32_UINT == GetIBFormat10( i ) )
            Options |= D3DXFROMWINE10_MESH_32_BIT;
        V_RETURN( D3DXFROMWINE10CreateMesh( pd3dDevice,
                                    layout,
                                    2,
        ...
        ID3DXFROMWINE10MeshBuffer* pIndexBuffer = NULL;
        ...
        //release the ID3DXFROMWINE10Mesh
        SAFE_RELEASE( pMesh );
    }
...
//--------------------------------------------------------------------------------------
void CDXUTSDKMesh::RenderAdjacent( 
                                   ID3D10EffectTechnique* pTechnique,
                                   ID3D10EffectShaderResourceVariable* ptxDiffuse,
                                   ID3D10EffectShaderResourceVariable* ptxNormal,
                                   ID3D10EffectShaderResourceVariable* ptxSpecular,
...
{
    WCHAR strPath[MAX_PATH];
    LPD3DXFROMWINEBUFFER pAdjacencyBuffer = NULL;
    LPD3DXFROMWINEBUFFER pMtrlBuffer = NULL;
    HRESULT hr;

    // Cleanup previous mesh if any
    Destroy();

    // Find the path for the file, and convert it to ANSI (for the D3DXFROMWINE API)
    DXUTFindDXSDKMediaFileCch( strPath, sizeof( strPath ) / sizeof( WCHAR ), strFilename );

    // Load the mesh
    if( FAILED( hr = D3DXFROMWINELoadMeshFromX( strPath, D3DXFROMWINEMESH_MANAGED, pd3dDevice,
    ...
    if( FAILED( hr = m_pMesh->OptimizeInplace(
                D3DXFROMWINEMESHOPT_COMPACT | D3DXFROMWINEMESHOPT_ATTRSORT | D3DXFROMWINEMESHOPT_VERTEXCACHE,
...
HRESULT CDXUTXFileMesh::Create( LPDIRECT3DDEVICE9 pd3dDevice,
                                LPD3DXFROMWINEFILEDATA pFileData )
{
    LPD3DXFROMWINEBUFFER pMtrlBuffer = NULL;
    LPD3DXFROMWINEBUFFER pAdjacencyBuffer = NULL;
    HRESULT hr;
    ...
    if( FAILED( hr = D3DXFROMWINELoadMeshFromXof( pFileData, D3DXFROMWINEMESH_MANAGED, pd3dDevice,
                                          &pAdjacencyBuffer, &pMtrlBuffer, NULL,
                                          &m_dwNumMaterials, &m_pMesh ) ) )
    {
        return hr;
...
//-----------------------------------------------------------------------------
HRESULT CDXUTXFileMesh::Create( LPDIRECT3DDEVICE9 pd3dDevice, ID3DXFROMWINEMesh* pInMesh,
                                D3DXFROMWINEMATERIAL* pd3dxMaterials, DWORD dwMaterials )
{
    // Cleanup previous mesh if any
    Destroy();
    ...
    dwOptions &= ~( D3DXFROMWINEMESH_32BIT | D3DXFROMWINEMESH_SYSTEMMEM | D3DXFROMWINEMESH_WRITEONLY );
    dwOptions |= D3DXFROMWINEMESH_MANAGED;
    dwOptions |= D3DXFROMWINEMESHOPT_COMPACT | D3DXFROMWINEMESHOPT_ATTRSORT | D3DXFROMWINEMESHOPT_VERTEXCACHE;

    ID3DXFROMWINEMesh* pTempMesh = NULL;
...
//-----------------------------------------------------------------------------
HRESULT CDXUTXFileMesh::CreateMaterials( LPCWSTR strPath, IDirect3DDevice9* pd3dDevice, D3DXFROMWINEMATERIAL* d3dxMtrls,
    ...
    if( d3dxMtrls && m_dwNumMaterials > 0 )
    {
        ...
        for( DWORD i = 0; i < m_dwNumMaterials; i++ )
        {
            ...
            if( d3dxMtrls[i].pTextureFilename )
            {
                ...
                D3DXFROMWINEIMAGE_INFO ImgInfo;
                ...
                switch( ImgInfo.ResourceType )
                {
                    case D3DRTYPE_TEXTURE:
                    {
                        ...
                        if( SUCCEEDED( D3DXFROMWINECreateTextureFromFile( pd3dDevice, strTexture, &pTex ) ) )
                        {
                    ...
                    case D3DRTYPE_CUBETEXTURE:
                    {
                        ...
                        if( SUCCEEDED( D3DXFROMWINECreateCubeTextureFromFile( pd3dDevice, strTexture, &pTex ) ) )
                        {
                            // Obtain the base texture interface
                            pTex->QueryInterface( IID_IDirect3DBaseTexture9, ( LPVOID* )&m_pTextures[i] );
                            // Release the specialized instance
                    ...
                    case D3DRTYPE_VOLUMETEXTURE:
                    {
                        ...
                        if( SUCCEEDED( D3DXFROMWINECreateVolumeTextureFromFile( pd3dDevice, strTexture, &pTex ) ) )
                        {
                            // Obtain the base texture interface
                            pTex->QueryInterface( IID_IDirect3DBaseTexture9, ( LPVOID* )&m_pTextures[i] );
                            // Release the specialized instance
...
{
    LPD3DXFROMWINEMESH pTempMesh = NULL;
    ...
    if( m_pMesh )
    {
        ...
        {
            D3DXFROMWINEComputeNormals( m_pMesh, NULL );
        }
    }
                                       ...
                                       bool bSplitVertexForOptimalTangents )
{
    ...
    {
        for( UINT index = 0; index < D3DXFROMWINEGetDeclLength( aOldDecl ); ++index )
        {
    ...
    {
        for( UINT index = 0; index < D3DXFROMWINEGetDeclLength( aNewDecl ); ++index )
        {
    ...
    if( pTempMesh )
    {
        ...
        if( bHaveNormalNow && !bHadTangent && bHaveTangentNow && bAutoComputeTangents )
        {
            ...
            hr = D3DXFROMWINEComputeTangentFrameEx( m_pMesh,
                                            D3DDECLUSAGE_TEXCOORD, 0,
                                            D3DDECLUSAGE_TANGENT, 0,
                                            D3DXFROMWINE_DEFAULT, 0,
...
//-----------------------------------------------------------------------------
HRESULT CDXUTXFileMesh::Render( ID3DXFROMWINEEffect* pEffect,
