...
bool CMudgeDib::Create(int a_Width, int a_Height, HDC a_DC)
{
	Destroy();
	...
	m_Bitmap = CreateDIBSection(a_DC, &m_Info, DIB_RGB_COLORS, (void **)&m_Bits, 0, 0); 
...
void CMudgeDib::Render(HDC a_DC, int a_X, int a_Y, int a_Width, int a_Height)
{
	StretchDIBits(a_DC, 
...
void CMudgeDib::Render(HDC a_DC, const CRect& a_Dest, const CRect& a_Source)
{
	StretchDIBits(a_DC, 
...
void CMudgeDib::SetPixel(int a_X, int a_Y, COLORREF a_Color)
{
	if (a_Y < 0 || a_X < 0)
		return;
...
COLORREF CMudgeDib::GetPixel(int a_X, int a_Y)
{
	if (a_Y < 0 || a_X < 0)
		return 0;
	if (a_Y >= m_Height || a_X >= m_Width)
