...
public:
	...
	bool		Create(int a_Width, int a_Height, HDC a_DC=0);
	bool		Destroy();
...
inline COLORREF* CMudgeDib::GetBits()
{
	return m_Bits;
}


inline COLORREF* CMudgeDib::GetScanline(int a_Y)
{
	return m_Bits + (a_Y*m_Width);
}


inline int CMudgeDib::GetWidth() const
{
	return m_Width;
}


inline int CMudgeDib::GetHeight() const
{
	return m_Height;
}


inline int CMudgeDib::GetPitch() const
{
	return m_Width*4;
}
