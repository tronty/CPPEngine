...
#else
#define __stdmin__ std::min
#define __stdmax__ std::max
#endif
...
bool TIntermBinary::promote(TParseContext& ctx)
{
   ...
   {
       cols = __stdmin__(left->getColsCount(), right->getColsCount());
       rows = __stdmin__(left->getRowsCount(), right->getRowsCount());
   }
   else
   {
       cols = __stdmax__(left->getColsCount(), right->getColsCount());
       rows = __stdmax__(left->getRowsCount(), right->getRowsCount());
   }
   assert(cols > 0);
   assert(rows > 0);
