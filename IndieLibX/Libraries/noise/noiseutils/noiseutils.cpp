...
namespace noise
{
  ...
  namespace utils
  {
    ...
    inline noise::uint8_libnoise_ BlendChannel (const uint8_libnoise_ channel0,
      const uint8_libnoise_ channel1, float alpha)
    {
      float c0 = (float)channel0 / 255.0;
      float c1 = (float)channel1 / 255.0;
      return (noise::uint8_libnoise_)(((c1 * alpha) + (c0 * (1.0f - alpha))) * 255.0f);
    }
    ...
    // hope.)
    inline noise::uint8_libnoise_* UnpackFloat (noise::uint8_libnoise_* bytes, float value)
    {
      noise::uint8_libnoise_* pBytes = (noise::uint8_libnoise_*)(&value);
      bytes[0] = *pBytes++;
      bytes[1] = *pBytes++;
      bytes[2] = *pBytes++;
      bytes[3] = *pBytes++;
    ...
    inline noise::uint8_libnoise_* UnpackLittle16 (noise::uint8_libnoise_* bytes,
      noise::uint16 integer)
    {
      bytes[0] = (noise::uint8_libnoise_)((integer & 0x00ff)      );
      bytes[1] = (noise::uint8_libnoise_)((integer & 0xff00) >> 8 );
      return bytes;
    }

    // Unpacks a 32-bit integer value into four bytes in little endian format.
    inline noise::uint8_libnoise_* UnpackLittle32 (noise::uint8_libnoise_* bytes,
      noise::uint32 integer)
    {
      bytes[0] = (noise::uint8_libnoise_)((integer & 0x000000ff)      );
      bytes[1] = (noise::uint8_libnoise_)((integer & 0x0000ff00) >> 8 );
      bytes[2] = (noise::uint8_libnoise_)((integer & 0x00ff0000) >> 16);
      bytes[3] = (noise::uint8_libnoise_)((integer & 0xff000000) >> 24);
      return bytes;
    }
...
void WriterBMP::WriteDestFile ()
{
  ...
  noise::uint8_libnoise_* pLineBuffer = NULL;
  ...
  try {
    pLineBuffer = new noise::uint8_libnoise_[bufferSize];
  }
  catch (...) {
    throw noise::ExceptionOutOfMemory ();
  }
  ...
  for (int y = 0; y < height; y++) {
    ...
    noise::uint8_libnoise_* pDest   = pLineBuffer;
  ...
  const Color& backgroundColor, double lightValue) const
{
  ...
  // Rescale the color channels to the noise::uint8_libnoise_ (0..255) range and return
  // the new color.
  Color newColor (
    (noise::uint8_libnoise_)((noise::uint_libnoise_)(red   * 255.0) & 0xff),
    (noise::uint8_libnoise_)((noise::uint_libnoise_)(green * 255.0) & 0xff),
    (noise::uint8_libnoise_)((noise::uint_libnoise_)(blue  * 255.0) & 0xff),
  ...
  double bumpHeight) const
{
  ...
  // range.
  noise::uint8_libnoise_ xc, yc, zc;
  xc = (noise::uint8_libnoise_)((noise::uint_libnoise_)((floor)((vxc + 1.0) * 127.5)) & 0xff);
  yc = (noise::uint8_libnoise_)((noise::uint_libnoise_)((floor)((vyc + 1.0) * 127.5)) & 0xff);
  zc = (noise::uint8_libnoise_)((noise::uint_libnoise_)((floor)((vzc + 1.0) * 127.5)) & 0xff);
