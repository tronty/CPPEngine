...
protected:
	...
	int m_id;
public:
	unsigned int getComponentsCount() const{return getChannelCount(format);}
	FORMAT format;
	...
	ubyte *create(const FORMAT fmt, const int w, const int h, const int d, const int mipMapCount, const int arraysize = 1, D3DXFROMWINEVECTOR3* aColor=0);
	void free();
	void clear();

	void setComponentsCount(const int N)
	{
		if(N==4)
			removeChannels(true, true, true, true);
		else if(N==3)
			removeChannels(true, true, true, false);
		else if(N==2)
			removeChannels(true, true, false, false);
		else if(N==1)
			removeChannels(true, false, false, false);
	}
#if 0
	void setInternalFormat(FORMAT aFormat){format=aFormat;}
	void setFormat(FORMAT aFormat){format=aFormat;}
	void setHeight(const int aHeight){ height=aHeight;  }
	void setWidth(const int aWidth){ width=aWidth;  }
#endif
	void setDataBuffer(ubyte* src){setPixels(src);}

	ubyte *getDataBuffer() const { return getPixels(); }
	ubyte *getPixels() const { return (ubyte*) &pixels[0]; }
	...
	ubyte *getExtraData() const { return (ubyte*) &extraData[0]; }
	void setExtraData(void *data, const int nBytes){
		nExtraData = nBytes;
		stx_memcpy(&extraData[0], (ubyte *) data, nBytes);
	}
...
#else
	bool loadHDR(const char *fileName){return false;}
#endif // NO_HDR
#ifndef NO_JPEG
...
#endif // NO_HDR
#ifndef NO_TIFF
	bool saveTIFF(const char *fileName, const int quality);
#endif // NO_TIFF
#ifndef NO_JPEG
	...
	bool compress(const FORMAT destFormat);

	// Decompresses the compressed formats
	bool decompress();
	bool crossToCube();
	...
	bool generateMipMaps(const int mipMapCount = ALL_MIPMAPS);
	...
	bool dilate();
	bool erode();
	bool resize(const int w, const int h = 1, const int d = 1);
