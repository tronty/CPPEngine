...
typedef struct {
  INT32_JPEG_ put_buffer;		/* current bit-accumulation buffer */
			 ...
			 c_derived_tbl ** pdtbl)
{
  ...
  while (huffsize[p]) {
     ...
     */
    if (((INT32_JPEG_) code) >= (((INT32_JPEG_) 1) << si))
      ERREXIT(cinfo, JERR_BAD_HUFF_TABLE);
    code <<= 1;
    si++;
  }
...
/* Emit some bits; return TRUE if successful, FALSE if must suspend */
{
  ...
  register INT32_JPEG_ put_buffer = (INT32_JPEG_) code;
  ...
  put_buffer &= (((INT32_JPEG_) 1)<<size) - 1; /* mask off any extra bits in code */
