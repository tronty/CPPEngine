...
//include <FrameworkHLSLCg3/Terrain.h>
using namespace STX;
//--------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------
#if 0
bool CPackedFile::CreatePackedFile(  CHAR* szFileName,
...
//--------------------------------------------------------------------------------------
bool CPackedFile::LoadPackedFile( CHAR* szFileName, bool b64Bit, VArray <LEVEL_ITEM*>* pLevelItemArray )
{
    bool bRet = false;
    ...
    for( UINT i = 0; i < m_FileHeader.NumFiles; i += 4 )
    {
        ...
        stx_strlcpy( pLevelItem->szVBName,  m_pFileIndices[i].szFileName,260 );
        stx_strlcpy( pLevelItem->szIBName,  m_pFileIndices[i + 1].szFileName,260 );
        stx_strlcpy( pLevelItem->szDiffuseName,  m_pFileIndices[i + 2].szFileName,260 );
        stx_strlcpy( pLevelItem->szNormalName,  m_pFileIndices[i + 3].szFileName,260 );
...
void CPackedFile::EnsureChunkMapped( UINT64 iChunk )
{
    if( !m_pMappedChunks[iChunk].bInUse )
    {
        ...
        {
            LOG_PRINT( "File Chunk not Mapped!\n" );
        }
        m_ChunksMapped ++;
    }
...
//    MultiByteToWideChar( CP_ACP, 0, szFile, -1, str, MAX_PATH );
    ...
    return GetPackedFileInfo( szFile, pDataBytes );
}
*/
//--------------------------------------------------------------------------------------
...
bool CPackedFile::GetPackedFileInfo( CHAR* szFile, UINT* pDataBytes )
{
    ...
    {
        if( 0 == strcmp( szFile, m_pFileIndices[i].szFileName ) )
        {
            iFoundIndex = i;
            break;
        }
...
//    MultiByteToWideChar( CP_ACP, 0, szFile, -1, str, MAX_PATH );
    ...
    return GetPackedFile( szFile, ppData, pDataBytes );
}
*/
//--------------------------------------------------------------------------------------
