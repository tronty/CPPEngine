...
//--------------------------------------------------------------------------------------
HRESULT CAsyncLoader::AddWorkItem( LoaderID pDataLoader0, IDataProcessor* pDataProcessor, HRESULT* pHResult,
                                   void** ppDeviceObject )
{
	IDataLoader* pDataLoader=0;//???
...
void CAsyncLoader::WaitForAllItems()
{
    ...
    for(; ; )
    {
        // Only stx_exit when all resources are loaded
    ...
    while( !m_bDone )
    {
        ...
        if( !ResourceRequest.bCopy )
        {
            if( !ResourceRequest.bError )
            {
                ...
                if( FAILED( hr ) )
                {
                    sprintf_s( szMessage, MAX_PATH, "FileIO Error: hr = %x\n", hr );
                    LOG_PRINT( szMessage );
        ...
        else
        {
            if( !ResourceRequest.bError )
            {
                ...
                if( FAILED( hr ) )
                {
                    LOG_PRINT( "Failed to Copy Data to Device Object\n" );
...
unsigned int CAsyncLoader::ProcessingThreadProc()
{
    ...
    while( !m_bDone )
    {
        ...
        if( FAILED( hr ) )
        {
            sprintf_s( szMessage, MAX_PATH, "Processing Thread Error: hr = %x\n", hr );
            LOG_PRINT( szMessage );
...
// it either Locks or Unlocks a resource .  One of
